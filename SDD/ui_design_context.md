...
- **歡迎頁面佈局**: 採用 **Row Layout (橫向排列)**，左側為立體書籍封面 (固定寬度 280px)，右側為歡迎語與功能導讀，僅在寬度極窄時才堆疊顯示。
...
### 2.3 歡迎頁面與品牌首頁 (Welcome & Landing Screen) - [NEW]
- **定位**: 位於內容面板 (Content Panel) 的初始狀態，作為使用者的第一接觸點。
- **佈局策略**:
    - **Desktop (>768px)**: 嚴格執行 **CSS Grid Row Layout** (`280px 1fr`)，確保「左圖右文」的視覺動線。
        - **左側 (Brand)**: 書籍立體封面 (Radius: 12px, Shadow: 40px)，固定寬度 280px。
        - **右側 (Content)**: 標題、Slogan、章節導讀與購買連結。
    - **Mobile (<768px)**: 自動切換為 **Flex Column Layout**，內容置中堆疊以適應窄屏閱讀。
- **視覺重點**: 
    - **玻璃容器**: 使用高強度的 `Glassmorphism` (`backdrop-filter: blur(20px)`) 區隔背景，創造懸浮感。
    - **轉化按鈕**: 購買連結採用 **Primary Blue Gradient** (`linear-gradient(135deg, #3b82f6, #2563eb)`) 與強陰影，作為頁面的視覺錨點 (Visual Anchor)。

### 2.4 滾動規範 (Scrollbar Mechanics)
- **極簡主義**: 全面移除瀏覽器預設的粗重滾動條。
- **自定義樣式**: 採用寬度僅 `6px` 的半透明細長設計，邊角全圓潤化，使其像是一層浮在界面上的交互元素，不破壞玻璃擬態的穿透感。

### 2.5 面板一致性 (Panel Harmonization) - [NEW]
- **雙生玻璃 (Twin Glass)**: 左側內容面板與右側代碼面板採用完全一致的視覺容器規範。
    - **半透明度**: `bg-opacity: 0.7` (Light) / `0.7` (Dark)。
    - **圓角統一**: 四個角落皆採用 `radius-xl` (20px)，創造懸浮卡片感。
    - **陰影同步**: 使用相同的 `box-shadow` 參數，確保兩者在 Z 軸上的視覺高度一致。

## 3. 響應式策略 (Responsive Strategy)
- **斷點**: **768px** (Tablet/Mobile Boundary)。
- **響應式行為**: 
    - **Welcome Screen**: 在 768px 以上堅持「左圖右文」佈局；768px 以下自動切換為垂直堆疊 (Column Layout) 並置中內容。
