...
try:
    ...
    for func in ['irr', 'npv', 'pmt', 'pv', 'rate', 'nper', 'fv', 'ppmt', 'ipmt']:
        if not hasattr(np, func) and hasattr(npf, func): setattr(np, func, getattr(npf, func))
                ...
                `)]),await e.yieldToUI(),e.update(95,"🔬 系統：正在安裝 SciPy 純 Python 計算引擎..."),await e.yieldToUI(),await t.runPythonAsync(Ue),e.update(98,"🚀 系統：正在啟動 Pymoo & QuantLib 虛擬層..."),await e.yieldToUI(),await Promise.all([t.runPythonAsync(He),t.runPythonAsync(We),t.runPythonAsync(_e),t.runPythonAsync(ge),t.runPythonAsync(Qe)]),e.update(99,"🛡️ 系統：權限校正與輸入掛鉤注入..."),await t.runPythonAsync(`
import builtins
import js
def custom_input(prompt = ""):
    try:
...
builtins.input = custom_input
            `),await e.yieldToUI(),X=t,$=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw $=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),$)}async function Je(s,e,t=3e4){let n;const l=new Promise((c,o)=>{n=setTimeout(()=>{o(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),l])}finally{clearTimeout(n)}}async function Ze(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
                `)}catch{}}const oe=new Set;async function fe(s,e){!s||!e||oe.has(e)||oe.add(e)}async function et(s){if(s)try{await s.loadPackage("micropip");const e=s.pyimport("micropip"),t=["/pwa_FRM_Book1/wheels/pymoo-0.4.1-py3-none-any.whl","/pwa_FRM_Book1/wheels/pandas_datareader-0.10.0-py3-none-any.whl","/pwa_FRM_Book1/wheels/pyodide_http-0.2.2-py3-none-any.whl","/pwa_FRM_Book1/wheels/seaborn-0.13.2-py3-none-any.whl"];await e.install(t,{transient:!0}).catch(n=>{console.warn("preloadHeavyPackages pip install failed",n)})}catch(e){console.warn("preloadHeavyPackages error",e)}}async function tt(s){try{const e=await s.runPythonAsync(`
import matplotlib.pyplot as plt
len(plt.get_fignums())
...
`)}const k=["sys","os","io","time","timeit","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib","fractions","struct","operator","string","decimal","abc","enum","typing","textwrap"],q=["numpy","pandas","matplotlib","micropip","js","builtins","QuantLib","mcint","arch","mibian"],N=[...new Set(w.map(h=>{const z=h.trim().split(/\s+/);return z[0]==="from",z[1].split(".")[0]}))].filter(h=>!k.includes(h)&&!q.includes(h)).filter(h=>{var z;return!M.has(h)&&!((z=window.failedPackages)!=null&&z.has(h))});if(N.length===0)return;const D=[],W=[],G=new URL("/pwa_FRM_Book1/",window.location.origin).href,Pe=new Set(["scipy","statsmodels","scikit-learn","sympy"]),Q=h=>{if(h){if(typeof h=="string"&&(h.endsWith(".whl")||h.startsWith("http"))){W.push(h);return}Pe.has(h)?D.push(h):W.push(h)}},le={sklearn:["scipy","scikit-learn"],"scikit-learn":["scipy"],statsmodels:["scipy"],sympy:[],seaborn:["matplotlib","pandas","scipy","statsmodels"],pandas_datareader:["requests"]};if(N.forEach(h=>{const z=J[h];z?(Q(z.endsWith(".whl")?G+z:z),(me[h]||[]).forEach(j=>{const Y=me[j]||j;Q(Y.endsWith(".whl")?G+Y:Y)})):Q(h),le[h]&&le[h].forEach(S=>{if(!M.has(S)&&!q.includes(S)){const j=J[S]||S;Q(j.endsWith(".whl")?G+j:j)}})}),D.length>0||W.length>0)try{const h=[...new Set(D)],z=[...new Set(W)];p||g(S=>S+`正在動態載入所需套件 [${N.join(", ")}]...
...
await micropip.install(${JSON.stringify(z)}, keep_going=True)
          `)),N.includes("pandas_datareader")&&await s.runPythonAsync(ge),N.includes("matplotlib")&&(await de(s),ae||se("AGG")),N.forEach(S=>{M.add(S);const j=J[S];j&&!j.endsWith(".whl")&&M.add(j)}),p||g(S=>S+`✅ 套件載入完成。
`)}catch(h){if(console.warn("Dependency loading failed:",h),window.failedPackages||(window.failedPackages=new Set),N.forEach(z=>window.failedPackages.add(z)),!p){const z=h.message||String(h);if(z.includes("Can't find a pure Python 3 wheel")){const S=((ie=z.match(/for: '([^']+)'/))==null?void 0:ie[1])||"unknown";g(j=>j+`⚠️ 套件 "${S}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
`)}else g(S=>S+`⚠️ 套件載入出現問題，嘗試直接執行...
`)}}},be=async u=>{if(!(!s||U||!u)){H(!0),g(`執行中...
`),R([]);try{a&&a.id&&await fe(s,a.id),await re(u),I.start("run-code"),await Ze(s);const p=u.includes("matplotlib.widgets")||u.includes("Slider")||u.includes("Button");ye(p);const w=p?"module://matplotlib_pyodide.wasm_backend":"AGG";if(ae!==w&&(await de(s,p),se(w)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),p){const q=document.getElementById("pyodide-plot-container");q&&(q.innerHTML="",window.document.pyodideMplTarget=q)}try{await Je(s,u)}catch(q){const N=await s.runPythonAsync("sys.stdout.getvalue()"),D=ce(q);g((N?N+`
`:"")+D),H(!1),I.end("run-code");return}const k=await s.runPythonAsync("sys.stdout.getvalue()");if(g(k||"執行完成（無文字內容輸出 ）"),ee)await at(s);else{const q=await tt(s);R(q)}}catch(p){const w=ce(p);g(w),console.error(p)}finally{H(!1),I.end("run-code")}}},ve=async u=>{let p=u;if(u&&!u.content&&(p=await xe(u.id)),!!p&&(f(p),x(null),F(""),g(""),R([]),p&&p.examples&&s)){const w=p.examples.map(k=>k.code).join(`
`);re(w,!0)}};_.useEffect(()=>{s&&a&&a.id&&fe(s,a.id).catch(u=>console.error("Dataset lazy load failed",u))},[s,a]);const we=u=>{x(u),O(V()),g(""),R([])},ke=u=>{x(u),O(V()),g(""),R([])},qe=()=>{x(null),g(""),R([])},ze=t;return r.jsxs("div",{className:`app ${A?"dark":""}`,children:[r.jsxs("div",{className:"main-content",children:[r.jsxs("div",{className:"top-bar",children:[r.jsxs("div",{className:"top-bar-left",children:[r.jsx(Se,{size:20,className:"logo-icon"}),r.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),r.jsx(Re,{chapters:b,currentChapter:a,bodyContent:B,onChapterSelect:ve,currentScript:m,onScriptSelect:ke,selectedTopicId:L,onTopicSelect:F,loading:P}),r.jsx("div",{className:"top-bar-right",children:r.jsx("button",{className:"theme-toggle",onClick:()=>he(!A),title:A?"切換到亮色模式":"切換到暗色模式",children:A?r.jsx(Ee,{size:20}):r.jsx(Ne,{size:20})})})]}),ze&&r.jsx("div",{className:"hydration-overlay",children:r.jsxs("div",{className:"hydration-card",children:[r.jsxs("div",{className:"hydration-header",children:[r.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),r.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),r.jsx("div",{className:"hydration-progress-container",children:r.jsx("div",{className:"hydration-progress-bar",style:{width:`${l}%`}})}),r.jsxs("div",{className:"hydration-status",children:[r.jsx("span",{children:o}),r.jsxs("span",{children:[l,"%"]})]})]})}),r.jsxs("div",{className:"panes-container",children:[r.jsx("div",{className:"content-pane",children:r.jsx(Me,{chapter:a,bodyContent:B,onCodeClick:we,selectedTopicId:L,darkMode:A,output:v,isRunning:U,plotImages:E,onClearOutput:()=>{g(""),R([])}})}),r.jsx("div",{className:`preview-pane${m?"":" preview-pane--hidden"}`,style:{width:m?`${te}px`:0},children:m&&r.jsx(_.Suspense,{fallback:r.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:r.jsx(rt,{script:m,onClose:qe,onRun:be,isRunning:U,isLoading:t,output:v,images:E,isInteractive:ee,darkMode:A,width:te,onResize:O})})})]})]}),r.jsx($e,{pyodide:s}),r.jsx(Ie,{})]})}async function ot(){try{if(!("serviceWorker"in navigator))return;const s="frm_sw_cleanup_done_v1";if(localStorage.getItem(s))return;if(!navigator.serviceWorker.controller){localStorage.setItem(s,"1");return}console.info("[AUTO-CLEAN] Detected active Service Worker — cleaning caches and storage.");try{const e=await navigator.serviceWorker.getRegistrations();await Promise.all(e.map(t=>t.unregister().catch(()=>{}))),console.info("[AUTO-CLEAN] serviceWorker registrations unregistered.")}catch(e){console.warn("[AUTO-CLEAN] Failed to unregister service workers",e)}try{if("caches"in window){const e=await caches.keys();await Promise.all(e.map(t=>caches.delete(t).catch(()=>{}))),console.info("[AUTO-CLEAN] CacheStorage cleared:",e)}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear CacheStorage",e)}try{if("databases"in indexedDB){const e=await indexedDB.databases();await Promise.all(e.map(t=>indexedDB.deleteDatabase(t.name).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB databases deleted:",e.map(t=>t.name))}else{const e=["pyodide","emscripten-archives","idb-filesystem","file_storage"];await Promise.all(e.map(t=>indexedDB.deleteDatabase(t).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB fallback delete attempted for candidates.")}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear IndexedDB",e)}try{localStorage.clear(),sessionStorage.clear(),console.info("[AUTO-CLEAN] localStorage and sessionStorage cleared.")}catch(e){console.warn("[AUTO-CLEAN] Failed to clear storage",e)}try{localStorage.setItem(s,"1")}catch{}try{console.info("[AUTO-CLEAN] Reloading page to load fresh assets."),window.location.reload(!0)}catch(e){console.info("[AUTO-CLEAN] Reload skipped.",e)}}catch(s){console.warn("[AUTO-CLEAN] Unexpected error",s)}}ot().catch(()=>{});je.createRoot(document.getElementById("root")).render(r.jsx(Le.StrictMode,{children:r.jsx(it,{})}));
