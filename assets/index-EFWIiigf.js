...
builtins.input = custom_input
            `),await e.yieldToUI(),Y=t,$=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw $=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),$)}async function Ke(s,e,t=3e4){let n;const l=new Promise((c,o)=>{n=setTimeout(()=>{o(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),l])}finally{clearTimeout(n)}}async function Je(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
                `)}catch{}}const re=new Set;async function le(s,e){!s||!e||re.has(e)||re.add(e)}async function Xe(s){if(s)try{await s.loadPackage("micropip");const e=s.pyimport("micropip"),t=["/pwa_FRM_Book1_python/wheels/pymoo-0.4.1-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/pandas_datareader-0.10.0-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/pyodide_http-0.2.2-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/seaborn-0.13.2-py3-none-any.whl"];await e.install(t,{transient:!0}).catch(n=>{console.warn("preloadHeavyPackages pip install failed",n)})}catch(e){console.warn("preloadHeavyPackages error",e)}}async function Ze(s){try{const e=await s.runPythonAsync(`
import matplotlib.pyplot as plt
len(plt.get_fignums())
...
`)}const z=["sys","os","io","time","timeit","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib","fractions","struct","operator","string","decimal","abc","enum","typing","textwrap"],P=["numpy","pandas","matplotlib","micropip","js","builtins","QuantLib","mcint","arch","mibian"],L=[...new Set(q.map(h=>{const w=h.trim().split(/\s+/);return w[0]==="from",w[1].split(".")[0]}))].filter(h=>!z.includes(h)&&!P.includes(h)).filter(h=>{var w;return!M.has(h)&&!((w=window.failedPackages)!=null&&w.has(h))});if(L.length===0)return;const D=[],O=[],H=new URL("/pwa_FRM_Book1_python/",window.location.origin).href,qe=new Set(["scipy","statsmodels","scikit-learn","sympy"]),Q=h=>{if(h){if(typeof h=="string"&&(h.endsWith(".whl")||h.startsWith("http"))){O.push(h);return}qe.has(h)?D.push(h):O.push(h)}},se={sklearn:["scipy","scikit-learn"],"scikit-learn":["scipy"],statsmodels:["scipy"],sympy:[],seaborn:["matplotlib","pandas","scipy","statsmodels"],pandas_datareader:["requests"]};if(L.forEach(h=>{const w=K[h];w?(Q(w.endsWith(".whl")?H+w:w),(de[h]||[]).forEach(N=>{const G=de[N]||N;Q(G.endsWith(".whl")?H+G:G)})):Q(h),se[h]&&se[h].forEach(S=>{if(!M.has(S)&&!P.includes(S)){const N=K[S]||S;Q(N.endsWith(".whl")?H+N:N)}})}),D.length>0||O.length>0)try{const h=[...new Set(D)],w=[...new Set(O)];p||g(S=>S+`正在動態載入所需套件 [${L.join(", ")}]...
...
await micropip.install(${JSON.stringify(w)}, keep_going=True)
          `)),L.includes("pandas_datareader")&&await s.runPythonAsync(me),L.includes("matplotlib")&&(await ie(s),Z||ee("AGG")),L.forEach(S=>{M.add(S);const N=K[S];N&&!N.endsWith(".whl")&&M.add(N)}),p||g(S=>S+`✅ 套件載入完成。
`)}catch(h){if(console.warn("Dependency loading failed:",h),window.failedPackages||(window.failedPackages=new Set),L.forEach(w=>window.failedPackages.add(w)),!p){const w=h.message||String(h);if(w.includes("Can't find a pure Python 3 wheel")){const S=((ne=w.match(/for: '([^']+)'/))==null?void 0:ne[1])||"unknown";g(N=>N+`⚠️ 套件 "${S}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
`)}else g(S=>S+`⚠️ 套件載入出現問題，嘗試直接執行...
`)}}},he=async _=>{if(!(!s||W||!_)){U(!0),g(`執行中...
`),R([]);try{a&&a.id&&await le(s,a.id),await ae(_),I.start("run-code"),await Je(s);const p=_.includes("matplotlib.widgets")||_.includes("Slider")||_.includes("Button");ue(p);const q=p?"module://matplotlib_pyodide.wasm_backend":"AGG";if(Z!==q&&(await ie(s,p),ee(q)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),p){const P=document.getElementById("pyodide-plot-container");P&&(P.innerHTML="",window.document.pyodideMplTarget=P)}try{await Ke(s,_)}catch(P){const L=await s.runPythonAsync("sys.stdout.getvalue()"),D=oe(P);g((L?L+`
`:"")+D),U(!1),I.end("run-code");return}const z=await s.runPythonAsync("sys.stdout.getvalue()");if(g(z||"執行完成（無文字內容輸出 ）"),J)await et(s);else{const P=await Ze(s);R(P)}}catch(p){const q=oe(p);g(q),console.error(p)}finally{U(!1),I.end("run-code")}}},xe=async _=>{let p=_;if(_&&!_.content&&(p=await ye(_.id)),!!p&&(f(p),x(null),F(""),g(""),R([]),p&&p.examples&&s)){const q=p.examples.map(z=>z.code).join(`
`);ae(q,!0)}};u.useEffect(()=>{s&&a&&a.id&&le(s,a.id).catch(_=>console.error("Dataset lazy load failed",_))},[s,a]);const be=_=>{x(_),g(""),R([])},ve=_=>{x(_),g(""),R([])},we=()=>{x(null),g(""),R([])},ke=t;return r.jsxs("div",{className:`app ${A?"dark":""}`,children:[r.jsxs("div",{className:"main-content",children:[r.jsxs("div",{className:"top-bar",children:[r.jsxs("div",{className:"top-bar-left",children:[r.jsx(ze,{size:20,className:"logo-icon"}),r.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),r.jsx(Ae,{chapters:b,currentChapter:a,bodyContent:B,onChapterSelect:xe,currentScript:m,onScriptSelect:ve,selectedTopicId:j,onTopicSelect:F,loading:k}),r.jsx("div",{className:"top-bar-right",children:r.jsx("button",{className:"theme-toggle",onClick:()=>_e(!A),title:A?"切換到亮色模式":"切換到暗色模式",children:A?r.jsx(Pe,{size:20}):r.jsx(Se,{size:20})})})]}),ke&&r.jsx("div",{className:"hydration-overlay",children:r.jsxs("div",{className:"hydration-card",children:[r.jsxs("div",{className:"hydration-header",children:[r.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),r.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),r.jsx("div",{className:"hydration-progress-container",children:r.jsx("div",{className:"hydration-progress-bar",style:{width:`${l}%`}})}),r.jsxs("div",{className:"hydration-status",children:[r.jsx("span",{children:o}),r.jsxs("span",{children:[l,"%"]})]})]})}),r.jsxs("div",{className:"panes-container",children:[r.jsx("div",{className:"content-pane",children:r.jsx(Re,{chapter:a,bodyContent:B,onCodeClick:be,selectedTopicId:j,darkMode:A,output:v,isRunning:W,plotImages:E,onClearOutput:()=>{g(""),R([])}})}),r.jsx("div",{className:`preview-pane${m?"":" preview-pane--hidden"}`,style:{width:m?`${X}px`:0},children:m&&r.jsx(u.Suspense,{fallback:r.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:r.jsx(st,{script:m,onClose:we,onRun:he,isRunning:W,isLoading:t,output:v,images:E,isInteractive:J,darkMode:A,width:X,onResize:ge})})})]})]}),r.jsx(Te,{pyodide:s}),r.jsx(Be,{})]})}async function rt(){try{if(!("serviceWorker"in navigator))return;const s="frm_sw_cleanup_done_v1";if(localStorage.getItem(s))return;if(!navigator.serviceWorker.controller){localStorage.setItem(s,"1");return}console.info("[AUTO-CLEAN] Detected active Service Worker — cleaning caches and storage.");try{const e=await navigator.serviceWorker.getRegistrations();await Promise.all(e.map(t=>t.unregister().catch(()=>{}))),console.info("[AUTO-CLEAN] serviceWorker registrations unregistered.")}catch(e){console.warn("[AUTO-CLEAN] Failed to unregister service workers",e)}try{if("caches"in window){const e=await caches.keys();await Promise.all(e.map(t=>caches.delete(t).catch(()=>{}))),console.info("[AUTO-CLEAN] CacheStorage cleared:",e)}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear CacheStorage",e)}try{if("databases"in indexedDB){const e=await indexedDB.databases();await Promise.all(e.map(t=>indexedDB.deleteDatabase(t.name).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB databases deleted:",e.map(t=>t.name))}else{const e=["pyodide","emscripten-archives","idb-filesystem","file_storage"];await Promise.all(e.map(t=>indexedDB.deleteDatabase(t).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB fallback delete attempted for candidates.")}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear IndexedDB",e)}try{localStorage.clear(),sessionStorage.clear(),console.info("[AUTO-CLEAN] localStorage and sessionStorage cleared.")}catch(e){console.warn("[AUTO-CLEAN] Failed to clear storage",e)}try{localStorage.setItem(s,"1")}catch{}try{console.info("[AUTO-CLEAN] Reloading page to load fresh assets."),window.location.reload(!0)}catch(e){console.info("[AUTO-CLEAN] Reload skipped.",e)}}catch(s){console.warn("[AUTO-CLEAN] Unexpected error",s)}}rt().catch(()=>{});Ee.createRoot(document.getElementById("root")).render(r.jsx(Ne.StrictMode,{children:r.jsx(nt,{})}));
