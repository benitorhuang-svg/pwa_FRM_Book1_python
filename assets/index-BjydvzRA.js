...
import{r as f,j as r,C as V,_ as pe,B as Se,S as Ee,M as Ne,R as je,a as Le}from"./vendor-react-DBlZ22TX.js";import{B as Ae,p as $e}from"./vendor-utils-DahDrsZr.js";import{r as Re}from"./vendor-katex-CrEV6Ds-.js";import"./vendor-B3CQIlJd.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const c of i)if(c.type==="childList")for(const o of c.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(i){const c={};return i.integrity&&(c.integrity=i.integrity),i.referrerPolicy&&(c.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?c.credentials="include":i.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function n(i){if(i.ep)return;i.ep=!0;const c=t(i);fetch(i.href,c)}})();function Me({chapters:s,currentChapter:e,bodyContent:t,onChapterSelect:n,currentScript:i,onScriptSelect:c,selectedTopicId:o,onTopicSelect:u,loading:b}){var k,p;const l=f.useMemo(()=>{const a=e==null?void 0:e.content;if(!a)return[];let d="";const y=a.intro;typeof y=="string"?d=y:y&&typeof y=="object"&&y.body&&(d+=typeof y.body=="string"?y.body:Object.values(y.body).join(`
`));const w=t||a.body;if(w&&(typeof w=="string"?d+=`
`+w:Array.isArray(w)?d+=`
`+w.join(`
`):typeof w=="object"&&(d+=`
`+Object.values(w).join(`
`))),!d)return[];const A=/^###\s+(.+)$/gm,R=[];let T;for(;(T=A.exec(d))!==null;){const D=T[1].trim(),B="topic-"+D.replace(/\s+/g,"-").toLowerCase();R.push({id:B,title:D})}return R},[e,t]);return r.jsxs("div",{className:"top-nav-container",children:[r.jsx("div",{className:"nav-group",children:r.jsxs("div",{className:"custom-select-wrapper",children:[r.jsxs("select",{id:"chapter-select",name:"chapter-select",value:(e==null?void 0:e.id)||"",onChange:a=>{const d=s.find(y=>y.id===a.target.value);d&&n(d)},disabled:b||s.length===0,className:"custom-select",children:[r.jsx("option",{value:"",disabled:!0,children:b?"載入中...":"📖 章節選擇"}),s.map(a=>r.jsx("option",{value:a.id,children:a.title},a.id))]}),r.jsx(V,{className:"select-icon",size:16})]})}),r.jsx("div",{className:`nav-group ${!e||l.length===0?"disabled":""}`,children:r.jsxs("div",{className:"custom-select-wrapper",children:[r.jsxs("select",{id:"topic-select",name:"topic-select",value:o,onChange:a=>u(a.target.value),disabled:!e||l.length===0,className:"custom-select",children:[r.jsx("option",{value:"",children:"💡 重點導覽"}),l.map(a=>r.jsx("option",{value:a.id,children:a.title},a.id))]}),r.jsx(V,{className:"select-icon",size:16})]})}),r.jsx("div",{className:`nav-group ${e?"":"disabled"}`,children:r.jsxs("div",{className:"custom-select-wrapper",children:[r.jsxs("select",{id:"script-select",name:"script-select",value:(i==null?void 0:i.filename)||"",onChange:a=>{var y;const d=(e==null?void 0:e.examples)||((y=e==null?void 0:e.content)==null?void 0:y.examples);if(d){const w=d.find(A=>A.filename===a.target.value);w&&c(w)}},disabled:!e||!(e.examples||(k=e.content)!=null&&k.examples)||(e.examples||((p=e.content)==null?void 0:p.examples)||[]).length===0,className:"custom-select",children:[r.jsx("option",{value:"",disabled:!0,children:"💻 程式代碼"}),(()=>{var y;return[...(e==null?void 0:e.examples)||((y=e==null?void 0:e.content)==null?void 0:y.examples)||[]].sort((w,A)=>w.filename.localeCompare(A.filename,void 0,{numeric:!0,sensitivity:"base"})).map(w=>r.jsx("option",{value:w.filename,children:w.filename},w.filename))})()]}),r.jsx(V,{className:"select-icon",size:16})]})})]})}const De=new Ae,qe=f.memo(({chapter:s,bodyContent:e,onCodeClick:t,selectedTopicId:n,output:i,isRunning:c,plotImages:o})=>{const u=f.useRef(null),b=f.useMemo(()=>{var k,p;if(!s)return null;const l=(k=s.content)==null?void 0:k.intro;if(l){let a="";if(typeof l=="string")a=l;else if(typeof l=="object"){a=`# ${l.title||""}
...
`));const v=e||((p=s.content)==null?void 0:p.body)||l.body;if(v)if(a+=`
...
`);const d=[];a=a.replace(/\$\$([\s\S]*?)\$\$/g,(v,g)=>{const j=d.length;return d.push({type:"display",content:g}),` @@MATH_BLOCK_${j}@@ `}),a=a.replace(/\\begin\{aligned\}([\s\S]*?)\\end\{aligned\}/g,(v,g)=>{const j=d.length;return d.push({type:"display",content:`\\begin{aligned}${g}\\end{aligned}`}),` @@MATH_BLOCK_${j}@@ `}),a=a.replace(new RegExp("(?<!\\\\)\\$([^$\\n]+?)\\$","g"),(v,g)=>{const j=d.length;return d.push({type:"inline",content:g}),` @@MATH_BLOCK_${j}@@ `});let w=De.parse(a).replace(/<h3>(.*?)<\/h3>/g,(v,g)=>`<h3 id="${"topic-"+g.replace(/<[^>]*>/g,"").trim().replace(/\s+/g,"-").toLowerCase()}">${g}</h3>`);const A=$e.sanitize(w,{USE_PROFILES:{html:!0,mathml:!0},ADD_TAGS:["math","annotation","semantics","mrow","msub","msup","msubsup","mover","munder","munderover","mmultiscripts","mprec","mnext","mtable","mtr","mtd","mfrac","msqrt","mroot","mstyle","merror","mpadded","mphantom","mfenced","menclose","ms","mglyph","maligngroup","malignmark","maction","svg","path","use","span","div"],ADD_ATTR:["id","target","xlink:href","class","style","aria-hidden","viewBox","d","fill","stroke","stroke-width","data-filename","encoding","display"]}),T=[...s.examples||[]].sort((v,g)=>g.filename.length-v.filename.length);let D=A;return T.forEach(v=>{const g=v.filename.replace(".","\\."),j=new RegExp(`(?<!['".\\w])(${g})(?!['".\\w])`,"g");D=D.replace(j,`<span class="code-link" data-filename="${v.filename}">${v.filename}</span>`)}),D.replace(/@@MATH_BLOCK_(\d+)@@/g,(v,g)=>{const j=d[parseInt(g)];return j.type==="display"?`\\[ ${j.content.trim()} \\]`:`\\( ${j.content.trim()} \\)`})}else{const a=s.examples||[];let d=`
        <div class="chapter-intro">
          <h2>${s.title}</h2>
          <p>本章包含 ${a.length} 個程式範例</p>
          <div class="example-grid">
      `;return a.forEach((y,w)=>{d+=`
      ...
      `,d}},[s,e]);return f.useEffect(()=>{const l=k=>{if(k.target.classList.contains("code-link")){const p=k.target.dataset.filename;let a=null;s!=null&&s.examples&&(a=s.examples.find(d=>d.filename===p),a&&!a.metadata&&(a.metadata={description:a.title})),a&&t(a)}};return document.addEventListener("click",l),()=>document.removeEventListener("click",l)},[s,t]),f.useEffect(()=>{const l=document.querySelector(".content-scroll");l&&(l.scrollTop=0)},[s]),f.useEffect(()=>{u.current&&b&&Re(u.current,{delimiters:[{left:"$$",right:"$$",display:!0},{left:"$",right:"$",display:!1},{left:"\\(",right:"\\)",display:!1},{left:"\\[",right:"\\]",display:!0}],throwOnError:!1})},[b]),f.useEffect(()=>{if(n){const l=document.getElementById(n);l&&l.scrollIntoView({behavior:"smooth",block:"start"})}},[n]),f.useEffect(()=>{if(i||o&&o.length>0||c){const l=document.querySelector(".content-scroll");l&&l.scrollTo({top:0,behavior:"instant"})}},[i,o,c]),r.jsx("div",{className:"content-panel",children:r.jsx("div",{className:"content-scroll",children:i||o&&o.length>0||c?r.jsxs("div",{id:"execution-output",className:"execution-output-section",children:[c&&r.jsxs("div",{className:"running-indicator",children:[r.jsx("div",{className:"spinner"}),r.jsx("span",{children:"程式執行中..."})]}),i&&r.jsx("pre",{className:"output-text",children:i}),o&&o.length>0&&r.jsx("div",{className:"output-images",children:o.map((l,k)=>r.jsx("div",{className:"output-image",children:r.jsx("img",{src:l,alt:`Plot ${k+1}`})},k))})]}):b?r.jsx("div",{ref:u,className:"markdown-body",dangerouslySetInnerHTML:{__html:b}}):r.jsx("div",{className:"welcome-screen",children:r.jsxs("div",{className:"welcome-card premium-welcome",children:[r.jsxs("div",{className:"welcome-brand",children:[r.jsx("img",{src:"book-cover.jpg",alt:"FRM Python 理論與實戰",className:"welcome-book-img"}),r.jsx("a",{href:"https://deepwisdom.com.tw/product/%e6%89%8b%e8%a1%93%e5%88%80%e8%88%ac%e7%b2%be%e6%ba%96%e7%9a%84frm-%e7%94%a8python%e7%a7%91%e5%ad%b8%e7%ae%a1%e6%8e%a7%e8%b2%a1%e9%87%91%e9%a2%a8%e9%9a%aa%e5%9f%ba%e7%a4%8e%e7%af%87dm2301/",target:"_blank",rel:"noopener noreferrer",className:"welcome-purchase-btn",children:"[ 本書官網購買連結 ]"})]}),r.jsxs("div",{className:"welcome-content",children:[r.jsxs("h2",{className:"welcome-title",children:["手術刀般精準的 FRM 用 Python",r.jsx("br",{}),"科學管控財金風險：基礎篇"]}),r.jsx("div",{className:"welcome-slogan",children:"☆★☆★【有如手術刀般精準！利用Python幫你管控財金風險！】★☆★☆"}),r.jsxs("div",{className:"welcome-text-scroll",children:[r.jsx("p",{children:"本書使用了當紅的程式語言 Python，從程式設計的基本觀念談起。沒有 Python 基礎也沒關係，完整的 Python 介紹，讓您能順利銜接資料科學家最常用的套件整理，包括 Numpy，以及特別針對格式化表格類處理的 Pandas，也充分介紹了 Dataframe 的各種應用。"}),r.jsx("p",{children:"在有了充足的資料之後，接著需要有可以展示數據的工具。除了大家最愛用的 Matplotlib 之外，也介紹了高手才會用的 Seaborn。當熟悉了工具之後，就正式進入了金融理論，包括基礎的機率及統計、各種模型及機率分佈，以及抽樣、信賴區間等內容說明。"}),r.jsx("p",{children:"最後則進入到金融領域，除了介紹各種計算的演算法、模型、術語，也結合了前面所學的 Python 及工具，並講解金融商品最重要的「固定收益分析」。"}),r.jsx("p",{children:"本書從科學下手，讓您精準了解金融原理，確保金錢不再陷入水深火熱之中，將是您從科學到金融領域最重要的橋樑。"})]})]})]})})})})});function Te(){return null}function Ce(s={}){const{immediate:e=!1,onNeedRefresh:t,onOfflineReady:n,onRegistered:i,onRegisteredSW:c,onRegisterError:o}=s;let u,b,l;const k=async(a=!0)=>{await b,l==null||l()};async function p(){if("serviceWorker"in navigator){if(u=await pe(async()=>{const{Workbox:a}=await import("./vendor-B3CQIlJd.js").then(d=>d.w);return{Workbox:a}},[]).then(({Workbox:a})=>new a("/pwa_FRM_Book1_python/sw.js",{scope:"/pwa_FRM_Book1_python/",type:"classic"})).catch(a=>{o==null||o(a)}),!u)return;l=()=>{u==null||u.messageSkipWaiting()};{let a=!1;const d=()=>{a=!0,u==null||u.addEventListener("controlling",y=>{y.isUpdate&&window.location.reload()}),t==null||t()};u.addEventListener("installed",y=>{typeof y.isUpdate>"u"?typeof y.isExternal<"u"&&y.isExternal?d():!a&&(n==null||n()):y.isUpdate||n==null||n()}),u.addEventListener("waiting",d)}u.register({immediate:e}).then(a=>{c?c("/pwa_FRM_Book1_python/sw.js",a):i==null||i(a)}).catch(a=>{o==null||o(a)})}}return b=p(),k}function Fe(s={}){const{immediate:e=!0,onNeedRefresh:t,onOfflineReady:n,onRegistered:i,onRegisteredSW:c,onRegisterError:o}=s,[u,b]=f.useState(!1),[l,k]=f.useState(!1),[p]=f.useState(()=>Ce({immediate:e,onOfflineReady(){k(!0),n==null||n()},onNeedRefresh(){b(!0),t==null||t()},onRegistered:i,onRegisteredSW:c,onRegisterError:o}));return{needRefresh:[u,b],offlineReady:[l,k],updateServiceWorker:p}}function Be(){const{offlineReady:[s,e],needRefresh:[t,n],updateServiceWorker:i}=Fe({onRegisteredSW(o,u){console.debug(`Service Worker at: ${o}`),u&&setInterval(()=>{u.update()},36e5)},onRegisterError(o){console.warn("SW registration error",o)}}),c=()=>{e(!1),n(!1)};return r.jsx("div",{className:"ReloadPrompt-container",children:(s||t)&&r.jsxs("div",{className:"ReloadPrompt-toast",children:[r.jsx("div",{className:"ReloadPrompt-message",children:s?"App ready to work offline":"New content available, click on reload button to update."}),t&&r.jsx("button",{className:"ReloadPrompt-toast-button",onClick:()=>i(!0),children:"Reload"}),r.jsx("button",{className:"ReloadPrompt-toast-button",onClick:c,children:"Close"})]})})}function Ie(s){const[e,t]=f.useState(null),[n,i]=f.useState(!1),c=f.useRef({});return f.useEffect(()=>{if(!s){t(null);return}const o=s.content,u=s.id;if(o!=null&&o.body&&!(o!=null&&o.bodyRef)){t(o.body);return}const b=o==null?void 0:o.bodyRef;if(!b||!Array.isArray(b)||b.length===0){t(null);return}let l=!1;return i(!0),(async()=>{const p={};await Promise.all(b.map(async a=>{const d=`${u}/${a}`;if(c.current[d]!==void 0){p[a]=c.current[d];return}try{const y=`/pwa_FRM_Book1_python/data/modular/${u}/${a}.json`,w=await fetch(y);if(!w.ok)throw new Error(`HTTP ${w.status}`);const R=(await w.json()).content||"";c.current[d]=R,p[a]=R}catch(y){console.warn(`[useBodyContent] Failed to load ${u}/${a}:`,y),p[a]=""}})),l||(t(p),i(!1))})(),()=>{l=!0}},[s]),{bodyContent:e,bodyLoading:n}}const me=`
# QuantLib (ql) 強大模擬層
...
builtins.input = custom_input
            `),await e.yieldToUI(),Y=t,F=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw F=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),F)}async function Je(s,e,t=3e4){let n;const i=new Promise((c,o)=>{n=setTimeout(()=>{o(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),i])}finally{clearTimeout(n)}}async function Ze(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
                `)}catch{}}const re=new Set;async function oe(s,e){if(!(!s||!e)&&!re.has(e))try{const t=`/pwa_FRM_Book1_python/data/datasets/${e}.zip`,n=await fetch(t);if(n.ok)if(console.log(`found dataset archive for ${e}`),window.JSZip){const i=await n.arrayBuffer(),c=await window.JSZip.loadAsync(i);await Promise.all(Object.keys(c.files).map(async o=>{const u=await c.files[o].async("uint8array");s.FS.writeFile(o,u)})),console.log(`datasets for ${e} extracted to FS`)}else console.warn("JSZip not available, cannot unzip datasets")}catch(t){console.warn("loadChapterDatasets error",t)}finally{re.add(e)}}async function Ke(s){if(s)try{await s.loadPackage("micropip");const e=s.pyimport("micropip"),t=["/pwa_FRM_Book1_python/wheels/pymoo-0.4.1-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/pandas_datareader-0.10.0-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/pyodide_http-0.2.2-py3-none-any.whl","/pwa_FRM_Book1_python/wheels/seaborn-0.13.2-py3-none-any.whl"];await e.install(t,{transient:!0}).catch(n=>{console.warn("preloadHeavyPackages pip install failed",n)})}catch(e){console.warn("preloadHeavyPackages error",e)}}async function Xe(s){try{const e=await s.runPythonAsync(`
import matplotlib.pyplot as plt
len(plt.get_fignums())
    `);if(e===0)return[];const t=[];for(let n=0;n<e;n++){const i=await s.runPythonAsync(`
import matplotlib.pyplot as plt
import io
import base64
...
img_base64
      `);t.push(`data:image/png;base64,${i}`)}return await s.runPythonAsync('import matplotlib.pyplot as plt; plt.close("all")'),t}catch(e){return console.error("Failed to capture plots:",e),[]}}async function ie(s,e=!1){try{const t=e?"module://matplotlib_pyodide.wasm_backend":"AGG";await s.runPythonAsync(`
import matplotlib
import matplotlib.pyplot as plt
matplotlib.use('${t}')
...
• 確認模組名稱正確
• 檢查是否支援該模組
• 查看完整錯誤訊息了解詳情`}];for(const{pattern:n,format:i}of t){const c=e.match(n);if(c)return i(c)}return`❌ 執行錯誤
...
• 逐行執行程式碼找出問題
• 使用 print() 輸出變數值
• 簡化程式碼，逐步測試`}class tt{constructor(){this.metrics={},this.enabled=!0}start(e){this.enabled&&(this.metrics[e]={start:performance.now(),end:null,duration:null})}end(e){if(!this.enabled||!this.metrics[e])return 0;const t=performance.now(),n=this.metrics[e].start,i=t-n;return this.metrics[e].end=t,this.metrics[e].duration=i,this.log(e,i),i}log(e,t){const n=t<100?"⚡":t<1e3?"⏱️":"🐌",i=t<100?"color: green":t<1e3?"color: orange":"color: red";console.log(`%c${n} ${e}: ${t.toFixed(2)}ms`,i),t>1e3&&console.warn(`⚠️ ${e} 執行時間過長: ${t.toFixed(2)}ms`)}async measure(e,t){this.start(e);try{return await t()}finally{this.end(e)}}getMetric(e){return this.metrics[e]||null}getMetrics(){return{...this.metrics}}getStats(){const e=Object.values(this.metrics).filter(t=>t.duration!==null).map(t=>t.duration);return e.length===0?{count:0,total:0,average:0,min:0,max:0}:{count:e.length,total:e.reduce((t,n)=>t+n,0),average:e.reduce((t,n)=>t+n,0)/e.length,min:Math.min(...e),max:Math.max(...e)}}clear(e){e?delete this.metrics[e]:this.metrics={}}setEnabled(e){this.enabled=e}report(){const e=this.getStats();console.group("📊 效能報告"),console.log(`總計測量: ${e.count} 次`),console.log(`總時間: ${e.total.toFixed(2)}ms`),console.log(`平均時間: ${e.average.toFixed(2)}ms`),console.log(`最快: ${e.min.toFixed(2)}ms`),console.log(`最慢: ${e.max.toFixed(2)}ms`),console.groupEnd(),console.group("📋 詳細指標"),Object.entries(this.metrics).forEach(([t,n])=>{n.duration!==null&&console.log(`${t}: ${n.duration.toFixed(2)}ms`)}),console.groupEnd()}}const z=new tt;z.setEnabled(!1);let ce=!1;function at(){if(!ce&&(ce=!0,"PerformanceObserver"in window)){new PerformanceObserver(i=>{const c=i.getEntries(),o=c[c.length-1];console.log("🎨 LCP:",o.renderTime||o.loadTime)}).observe({entryTypes:["largest-contentful-paint"]}),new PerformanceObserver(i=>{i.getEntries().forEach(o=>{console.log("⚡ FID:",o.processingStart-o.startTime)})}).observe({entryTypes:["first-input"]});let t=0;new PerformanceObserver(i=>{i.getEntries().forEach(o=>{o.hadRecentInput||(t+=o.value)}),console.log("📐 CLS:",t)}).observe({entryTypes:["layout-shift"]})}}const st=f.lazy(()=>pe(()=>import("./CodePreviewPanel-DcyHY-xF.js"),__vite__mapDeps([0,1,2,3]))),J={QuantLib:"QuantLib",arch:"arch",autograd:"autograd",chart_studio:"chart-studio",datetime:"datetime",lxml:"lxml",math:"math",matplotlib:"matplotlib",mcint:"mcint",mibian:"mibian",mpl_toolkits:"matplotlib",numpy:"numpy",numpy_financial:"wheels/numpy_financial-1.0.0-py3-none-any.whl",openpyxl:"openpyxl",pandas:"pandas",pandas_datareader:"wheels/pandas_datareader-0.10.0-py3-none-any.whl",plotly:"plotly",prettytable:"prettytable",pylab:"matplotlib",pymoo:"wheels/pymoo-0.4.1-py3-none-any.whl",pyodide_http:"wheels/pyodide_http-0.2.2-py3-none-any.whl",qpsolvers:"qpsolvers",requests:"requests","scikit-learn":"scikit-learn",scipy:"scipy",seaborn:"wheels/seaborn-0.13.2-py3-none-any.whl",sklearn:"scikit-learn",statsmodels:"statsmodels",sympy:"sympy",tabulate:"tabulate"},de={requests:["certifi","charset_normalizer","idna","urllib3"],certifi:"wheels/certifi-2026.1.4-py3-none-any.whl",charset_normalizer:"wheels/charset_normalizer-3.4.4-py3-none-any.whl",idna:"wheels/idna-3.11-py3-none-any.whl",urllib3:"wheels/urllib3-2.6.3-py3-none-any.whl"};typeof window<"u"&&"serviceWorker"in navigator&&(async()=>{try{const s=`/pwa_FRM_Book1_python/assets-manifest.json?t=${Date.now()}`,e=`/pwa_FRM_Book1_python/manifest.webmanifest?t=${Date.now()}`;let t=null;try{const o=await fetch(s,{cache:"no-store"});o&&o.ok&&(t=await o.text())}catch{}if(!t)try{const o=await fetch(e,{cache:"no-store"});if(!o||!o.ok)return;t=await o.text()}catch{return}const n=localStorage.getItem("app_assets_manifest_text");if(n&&n===t)return;localStorage.setItem("app_assets_manifest_text",t);const i=await navigator.serviceWorker.getRegistrations();if(!i||i.length===0){localStorage.getItem("app_manifest_reload_done")||(localStorage.setItem("app_manifest_reload_done","1"),location.reload());return}for(const o of i)try{await o.update(),o.waiting&&o.waiting.postMessage({type:"SKIP_WAITING"})}catch(u){console.warn("SW update attempt failed:",u)}const c=()=>{localStorage.getItem("app_manifest_reload_done")||(localStorage.setItem("app_manifest_reload_done","1"),window.location.reload())};navigator.serviceWorker.addEventListener("controllerchange",c)}catch(s){console.warn("Version check/update failed:",s)}})();function nt(){const[s,e]=f.useState(null),[t,n]=f.useState(!0),[i,c]=f.useState(0),[o,u]=f.useState("初始化中..."),[b,l]=f.useState([]),[k,p]=f.useState(!0),[a,d]=f.useState(null),[y,w]=f.useState(null),[A,R]=f.useState(""),[T,D]=f.useState({}),{bodyContent:B}=Ie(a),[v,g]=f.useState(""),[j,q]=f.useState([]),[W,U]=f.useState(!1),[Z,ue]=f.useState(!1),[M,ye]=f.useState(()=>{const h=localStorage.getItem("theme");return h==="dark"||!h&&!1}),[K,he]=f.useState(600),[C]=f.useState(new Set),[X,ee]=f.useState(null),te=f.useRef(!1);f.useEffect(()=>{p(!0);const h=`/pwa_FRM_Book1_python/data/chapters_index.json?t=${Date.now()}`;fetch(h).then(m=>m.json()).then(m=>{m&&m.length>0&&l(m),p(!1)}).catch(m=>{console.error("Failed to load chapters index:",m),p(!1)})},[]);const ge=async h=>{if(T[h])return T[h];try{p(!0);const m=await fetch(`/pwa_FRM_Book1_python/data/chapters_${h}.json?t=${Date.now()}`);if(!m.ok)throw new Error(`HTTP error! status: ${m.status}`);const P=await m.text();let S=null;try{S=JSON.parse(P)}catch{try{const E=P.replace(/\\(?!["\\/bfnrtu])/g,"\\\\");S=JSON.parse(E)}catch(E){throw new Error(`Failed to parse chapter data: ${E.message}`)}}return D(E=>({...E,[h]:S})),p(!1),S}catch(m){return console.error(`Failed to load chapter ${h}:`,m),p(!1),null}};f.useEffect(()=>{Ye((m,P)=>{c(m),u(P)}).then(m=>{e(m),n(!1),z.end("pyodide-init")}),at();const h=[];if("serviceWorker"in navigator){const m=setInterval(()=>{navigator.serviceWorker.getRegistrations().then(S=>{S.forEach(E=>E.update())})},36e5),P=()=>{};navigator.serviceWorker.addEventListener("controllerchange",P),h.push(()=>{clearInterval(m),navigator.serviceWorker.removeEventListener("controllerchange",P)})}if("storage"in navigator&&"estimate"in navigator.storage){const m=async()=>{try{const S=await navigator.storage.estimate();S.usage&&S.quota&&S.usage/S.quota*100>80&&console.warn("Storage usage is high (>80%). Consider cleaning up.")}catch(S){console.warn("Failed to check storage quota:",S)}};m();const P=setInterval(m,300*1e3);h.push(()=>clearInterval(P))}return()=>{h.forEach(m=>m())}},[]),f.useEffect(()=>{s&&!t&&Ke(s).then(()=>{te.current||(te.current=!0,console.warn("Background initialization complete"))}).catch(h=>console.error("Background loaded failed",h))},[s,t]),f.useEffect(()=>{localStorage.setItem("theme",M?"dark":"light"),document.documentElement.setAttribute("data-theme",M?"dark":"light")},[M]);const ae=async(h,m=!1)=>{var ne;if(!s)return;const P=h.match(/^\s*(?:from|import)\s+([a-zA-Z0-9_]+)/gm);if(!P)return;if(/^\s*(?:from|import)\s+QuantLib\b/m.test(h)&&!C.has("QuantLib"))try{m||g(_=>_+`正在啟用 QuantLib 相容性墊片...
`),await s.runPythonAsync(me),C.add("QuantLib"),m||g(_=>_+`✅ QuantLib 墊片已啟用。
`)}catch(_){console.warn("QuantLib shim injection failed:",_),m||g(x=>x+`⚠️ QuantLib 墊片啟用失敗，將嘗試繼續執行...
`)}const S=["sys","os","io","time","timeit","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib","fractions","struct","operator","string","decimal","abc","enum","typing","textwrap"],E=["numpy","pandas","matplotlib","micropip","js","builtins","QuantLib","mcint","arch","mibian"],$=[...new Set(P.map(_=>{const x=_.trim().split(/\s+/);return x[0]==="from",x[1].split(".")[0]}))].filter(_=>!S.includes(_)&&!E.includes(_)).filter(_=>{var x;return!C.has(_)&&!((x=window.failedPackages)!=null&&x.has(_))});if($.length===0)return;const I=[],O=[],H=new URL("/pwa_FRM_Book1_python/",window.location.origin).href,Pe=new Set(["scipy","statsmodels","scikit-learn","sympy"]),Q=_=>{if(_){if(typeof _=="string"&&(_.endsWith(".whl")||_.startsWith("http"))){O.push(_);return}Pe.has(_)?I.push(_):O.push(_)}},se={sklearn:["scipy","scikit-learn"],"scikit-learn":["scipy"],statsmodels:["scipy"],sympy:[],seaborn:["matplotlib","pandas","scipy","statsmodels"],pandas_datareader:["requests"]};if($.forEach(_=>{const x=J[_];x?(Q(x.endsWith(".whl")?H+x:x),(de[_]||[]).forEach(L=>{const G=de[L]||L;Q(G.endsWith(".whl")?H+G:G)})):Q(_),se[_]&&se[_].forEach(N=>{if(!C.has(N)&&!E.includes(N)){const L=J[N]||N;Q(L.endsWith(".whl")?H+L:L)}})}),I.length>0||O.length>0)try{const _=[...new Set(I)],x=[...new Set(O)];m||g(N=>N+`正在動態載入所需套件 [${$.join(", ")}]...
...
await micropip.install(${JSON.stringify(x)}, keep_going=True)
          `)),$.includes("pandas_datareader")&&await s.runPythonAsync(fe),$.includes("matplotlib")&&(await ie(s),X||ee("AGG")),$.forEach(N=>{C.add(N);const L=J[N];L&&!L.endsWith(".whl")&&C.add(L)}),m||g(N=>N+`✅ 套件載入完成。
...
`),q([]);try{a&&a.id&&await oe(s,a.id),await ae(h),z.start("run-code"),await Ze(s);const m=h.includes("matplotlib.widgets")||h.includes("Slider")||h.includes("Button");ue(m);const P=m?"module://matplotlib_pyodide.wasm_backend":"AGG";if(X!==P&&(await ie(s,m),ee(P)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),m){const E=document.getElementById("pyodide-plot-container");E&&(E.innerHTML="",window.document.pyodideMplTarget=E)}try{await Je(s,h)}catch(E){const $=await s.runPythonAsync("sys.stdout.getvalue()"),I=le(E);g(($?$+`
`:"")+I),U(!1),z.end("run-code");return}const S=await s.runPythonAsync("sys.stdout.getvalue()");if(g(S||"執行完成（無文字內容輸出 ）"),Z)await et(s);else{const E=await Xe(s);q(E)}}catch(m){const P=le(m);g(P),console.error(m)}finally{U(!1),z.end("run-code")}}},we=async h=>{let m=h;if(h&&!h.content&&(m=await ge(h.id)),!!m&&(d(m),w(null),R(""),g(""),q([]),m&&m.examples&&s)){const P=m.examples.map(S=>S.code).join(`
`);ae(P,!0)}};f.useEffect(()=>{s&&a&&a.id&&oe(s,a.id).catch(h=>console.error("Dataset lazy load failed",h))},[s,a]);const be=h=>{w(h),g(""),q([])},ve=h=>{w(h),g(""),q([])},xe=()=>{w(null),g(""),q([])},ke=t;return r.jsxs("div",{className:`app ${M?"dark":""}`,children:[r.jsxs("div",{className:"main-content",children:[r.jsxs("div",{className:"top-bar",children:[r.jsxs("div",{className:"top-bar-left",children:[r.jsx(Se,{size:20,className:"logo-icon"}),r.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),r.jsx(Me,{chapters:b,currentChapter:a,bodyContent:B,onChapterSelect:we,currentScript:y,onScriptSelect:ve,selectedTopicId:A,onTopicSelect:R,loading:k}),r.jsx("div",{className:"top-bar-right",children:r.jsx("button",{className:"theme-toggle",onClick:()=>ye(!M),title:M?"切換到亮色模式":"切換到暗色模式",children:M?r.jsx(Ee,{size:20}):r.jsx(Ne,{size:20})})})]}),ke&&r.jsx("div",{className:"hydration-overlay",children:r.jsxs("div",{className:"hydration-card",children:[r.jsxs("div",{className:"hydration-header",children:[r.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),r.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),r.jsx("div",{className:"hydration-progress-container",children:r.jsx("div",{className:"hydration-progress-bar",style:{width:`${i}%`}})}),r.jsxs("div",{className:"hydration-status",children:[r.jsx("span",{children:o}),r.jsxs("span",{children:[i,"%"]})]})]})}),r.jsxs("div",{className:"panes-container",children:[r.jsx("div",{className:"content-pane",children:r.jsx(qe,{chapter:a,bodyContent:B,onCodeClick:be,selectedTopicId:A,darkMode:M,output:v,isRunning:W,plotImages:j,onClearOutput:()=>{g(""),q([])}})}),y&&r.jsx("div",{className:"preview-pane",style:{width:`${K}px`},children:r.jsx(f.Suspense,{fallback:r.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:r.jsx(st,{script:y,onClose:xe,onRun:_e,isRunning:W,isLoading:t,output:v,images:j,isInteractive:Z,darkMode:M,width:K,onResize:he})})})]})]}),r.jsx(Te,{pyodide:s}),r.jsx(Be,{})]})}async function rt(){try{if(!("serviceWorker"in navigator))return;const s="frm_sw_cleanup_done_v1";if(localStorage.getItem(s))return;if(!navigator.serviceWorker.controller){localStorage.setItem(s,"1");return}console.info("[AUTO-CLEAN] Detected active Service Worker — cleaning caches and storage.");try{const e=await navigator.serviceWorker.getRegistrations();await Promise.all(e.map(t=>t.unregister().catch(()=>{}))),console.info("[AUTO-CLEAN] serviceWorker registrations unregistered.")}catch(e){console.warn("[AUTO-CLEAN] Failed to unregister service workers",e)}try{if("caches"in window){const e=await caches.keys();await Promise.all(e.map(t=>caches.delete(t).catch(()=>{}))),console.info("[AUTO-CLEAN] CacheStorage cleared:",e)}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear CacheStorage",e)}try{if("databases"in indexedDB){const e=await indexedDB.databases();await Promise.all(e.map(t=>indexedDB.deleteDatabase(t.name).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB databases deleted:",e.map(t=>t.name))}else{const e=["pyodide","emscripten-archives","idb-filesystem","file_storage"];await Promise.all(e.map(t=>indexedDB.deleteDatabase(t).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB fallback delete attempted for candidates.")}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear IndexedDB",e)}try{localStorage.clear(),sessionStorage.clear(),console.info("[AUTO-CLEAN] localStorage and sessionStorage cleared.")}catch(e){console.warn("[AUTO-CLEAN] Failed to clear storage",e)}try{localStorage.setItem(s,"1")}catch{}try{console.info("[AUTO-CLEAN] Reloading page to load fresh assets."),window.location.reload(!0)}catch(e){console.info("[AUTO-CLEAN] Reload skipped.",e)}}catch(s){console.warn("[AUTO-CLEAN] Unexpected error",s)}}rt().catch(()=>{});je.createRoot(document.getElementById("root")).render(r.jsx(Le.StrictMode,{children:r.jsx(nt,{})}));
