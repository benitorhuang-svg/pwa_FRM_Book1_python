...
builtins.input = custom_input
            `),await e.yieldToUI(),Q=t,D=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw D=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),D)}async function ze(s,e,t=3e4){let n;const o=new Promise((d,h)=>{n=setTimeout(()=>{h(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),o])}finally{clearTimeout(n)}}async function Fe(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
...
• 逐行執行程式碼找出問題
• 使用 print() 輸出變數值
• 簡化程式碼，逐步測試`}class Ae{constructor(){this.metrics={},this.enabled=!0}start(e){this.enabled&&(this.metrics[e]={start:performance.now(),end:null,duration:null})}end(e){if(!this.enabled||!this.metrics[e])return 0;const t=performance.now(),n=this.metrics[e].start,o=t-n;return this.metrics[e].end=t,this.metrics[e].duration=o,this.log(e,o),o}log(e,t){const n=t<100?"⚡":t<1e3?"⏱️":"🐌",o=t<100?"color: green":t<1e3?"color: orange":"color: red";console.log(`%c${n} ${e}: ${t.toFixed(2)}ms`,o),t>1e3&&console.warn(`⚠️ ${e} 執行時間過長: ${t.toFixed(2)}ms`)}async measure(e,t){this.start(e);try{return await t()}finally{this.end(e)}}getMetric(e){return this.metrics[e]||null}getMetrics(){return{...this.metrics}}getStats(){const e=Object.values(this.metrics).filter(t=>t.duration!==null).map(t=>t.duration);return e.length===0?{count:0,total:0,average:0,min:0,max:0}:{count:e.length,total:e.reduce((t,n)=>t+n,0),average:e.reduce((t,n)=>t+n,0)/e.length,min:Math.min(...e),max:Math.max(...e)}}clear(e){e?delete this.metrics[e]:this.metrics={}}setEnabled(e){this.enabled=e}report(){const e=this.getStats();console.group("📊 效能報告"),console.log(`總計測量: ${e.count} 次`),console.log(`總時間: ${e.total.toFixed(2)}ms`),console.log(`平均時間: ${e.average.toFixed(2)}ms`),console.log(`最快: ${e.min.toFixed(2)}ms`),console.log(`最慢: ${e.max.toFixed(2)}ms`),console.groupEnd(),console.group("📋 詳細指標"),Object.entries(this.metrics).forEach(([t,n])=>{n.duration!==null&&console.log(`${t}: ${n.duration.toFixed(2)}ms`)}),console.groupEnd()}}const I=new Ae;I.setEnabled(!1);let X=!1;function Re(){if(!X&&(X=!0,"PerformanceObserver"in window)){new PerformanceObserver(o=>{const d=o.getEntries(),h=d[d.length-1];console.log("🎨 LCP:",h.renderTime||h.loadTime)}).observe({entryTypes:["largest-contentful-paint"]}),new PerformanceObserver(o=>{o.getEntries().forEach(h=>{console.log("⚡ FID:",h.processingStart-h.startTime)})}).observe({entryTypes:["first-input"]});let t=0;new PerformanceObserver(o=>{o.getEntries().forEach(h=>{h.hadRecentInput||(t+=h.value)}),console.log("📐 CLS:",t)}).observe({entryTypes:["layout-shift"]})}}const Be=p.lazy(()=>me(()=>import("./CodePreviewPanel-BAmgNmSY.js"),__vite__mapDeps([0,1,2,3]))),ee={pandas:"pandas",matplotlib:"matplotlib",scipy:"scipy",statsmodels:"statsmodels",sympy:"sympy",autograd:"autograd",lxml:"lxml",openpyxl:"openpyxl",requests:"requests",numpy_financial:"wheels/numpy_financial-1.0.0-py3-none-any.whl",seaborn:"wheels/seaborn-0.13.2-py3-none-any.whl",pymoo:"wheels/pymoo-0.4.1-py3-none-any.whl",pandas_datareader:"wheels/pandas_datareader-0.10.0-py3-none-any.whl",pyodide_http:"wheels/pyodide_http-0.2.2-py3-none-any.whl",mpl_toolkits:"matplotlib",pylab:"matplotlib"},te={requests:["certifi","charset_normalizer","idna","urllib3"],certifi:"wheels/certifi-2026.1.4-py3-none-any.whl",charset_normalizer:"wheels/charset_normalizer-3.4.4-py3-none-any.whl",idna:"wheels/idna-3.11-py3-none-any.whl",urllib3:"wheels/urllib3-2.6.3-py3-none-any.whl"};function Qe(){const[s,e]=p.useState(null),[t,n]=p.useState(!0),[o,d]=p.useState(0),[h,r]=p.useState("初始化中..."),[y,c]=p.useState([]),[i,l]=p.useState(!0),[u,b]=p.useState(null),[S,w]=p.useState(null),[k,q]=p.useState(""),[F,x]=p.useState(""),[O,M]=p.useState([]),[C,T]=p.useState(!1),[U,ae]=p.useState(!1),[E,se]=p.useState(()=>{const f=localStorage.getItem("theme");return f==="dark"||!f&&!1}),[W,ne]=p.useState(600),[A]=p.useState(new Set),[H,G]=p.useState(null);p.useEffect(()=>{l(!0);const f=`/pwa_FRM_Book1_python/data/chapters.json?t=${Date.now()}`;fetch(f).then(m=>m.json()).then(m=>{m&&m.length>0&&c(m),l(!1)}).catch(m=>{console.error("Failed to load chapters:",m),l(!1)})},[]),p.useEffect(()=>{Ie((m,_)=>{d(m),r(_)}).then(m=>{e(m),n(!1),I.end("pyodide-init")}),Re();const f=[];if("serviceWorker"in navigator){const m=setInterval(()=>{navigator.serviceWorker.getRegistrations().then(P=>{P.forEach(N=>N.update())})},36e5),_=()=>{};navigator.serviceWorker.addEventListener("controllerchange",_),f.push(()=>{clearInterval(m),navigator.serviceWorker.removeEventListener("controllerchange",_)})}if("storage"in navigator&&"estimate"in navigator.storage){const m=async()=>{try{const P=await navigator.storage.estimate();P.usage&&P.quota&&P.usage/P.quota*100>80&&console.warn("Storage usage is high (>80%). Consider cleaning up.")}catch(P){console.warn("Failed to check storage quota:",P)}};m();const _=setInterval(m,300*1e3);f.push(()=>clearInterval(_))}return()=>{f.forEach(m=>m())}},[]),p.useEffect(()=>{localStorage.setItem("theme",E?"dark":"light"),document.documentElement.setAttribute("data-theme",E?"dark":"light")},[E]);const Y=async(f,m=!1)=>{var Z;if(!s)return;const _=f.match(/^\s*(?:from|import)\s+([a-zA-Z0-9_]+)/gm);if(!_)return;const P=["sys","os","io","time","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib"],N=["numpy","pandas","matplotlib","scipy","statsmodels","sympy","lxml","micropip","js","builtins","QuantLib"],j=[...new Set(_.map(v=>{const g=v.trim().split(/\s+/);return g[0]==="from",g[1].split(".")[0]}))].filter(v=>!P.includes(v)&&!N.includes(v)).filter(v=>!A.has(v));if(j.length===0)return;const $=[],V=new URL("/pwa_FRM_Book1_python/",window.location.origin).href;if(j.forEach(v=>{const g=ee[v];g?($.push(g.endsWith(".whl")?V+g:g),(te[v]||[]).forEach(z=>{const R=te[z]||z;$.push(R.endsWith(".whl")?V+R:R)})):$.push(v)}),$.length>0)try{const v=[...new Set($)];m||x(g=>g+`正在動態載入所需套件 [${j.join(", ")}]...
`),await s.loadPackage("micropip"),await s.runPythonAsync(`
import micropip
await micropip.install(${JSON.stringify(v)}, keep_going=True)
        `),j.includes("matplotlib")&&(await J(s),H||G("AGG")),j.forEach(g=>{A.add(g);const L=ee[g];L&&!L.endsWith(".whl")&&A.add(L)}),m||x(g=>g+`✅ 套件載入完成。
`)}catch(v){if(console.warn("Dependency loading failed:",v),!m){const g=v.message||String(v);if(g.includes("Can't find a pure Python 3 wheel")){const L=((Z=g.match(/for: '([^']+)'/))==null?void 0:Z[1])||"unknown";x(z=>z+`⚠️ 套件 "${L}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
`)}else x(L=>L+`⚠️ 套件載入出現問題，嘗試直接執行...
`)}}},re=async f=>{if(!(!s||C||!f)){I.start("run-code"),T(!0),x(`執行中...
`),M([]);try{await Fe(s);const m=f.includes("matplotlib.widgets")||f.includes("Slider")||f.includes("Button");ae(m);const _=m?"module://matplotlib_pyodide.wasm_backend":"AGG";if(H!==_&&(await J(s,m),G(_)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),m){const N=document.getElementById("pyodide-plot-container");N&&(N.innerHTML="",window.document.pyodideMplTarget=N)}await Y(f);try{await ze(s,f)}catch(N){const j=await s.runPythonAsync("sys.stdout.getvalue()"),$=K(N);x((j?j+`
`:"")+$),T(!1),I.end("run-code");return}const P=await s.runPythonAsync("sys.stdout.getvalue()");if(x(P||"執行完成（無文字內容輸出 ）"),U)await Te(s);else{const N=await Ce(s);M(N)}}catch(m){const _=K(m);x(_),console.error(m)}finally{T(!1),I.end("run-code")}}},oe=f=>{if(b(f),w(null),q(""),x(""),M([]),f&&f.examples&&s){const m=f.examples.map(_=>_.code).join(`
`);Y(m,!0)}},ie=f=>{w(f),x(""),M([])},le=f=>{w(f),x(""),M([])},de=()=>{w(null),x(""),M([])},ce=t;return a.jsxs("div",{className:`app ${E?"dark":""}`,children:[a.jsxs("div",{className:"main-content",children:[a.jsxs("div",{className:"top-bar",children:[a.jsxs("div",{className:"top-bar-left",children:[a.jsx(pe,{size:20,className:"logo-icon"}),a.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),a.jsx(be,{chapters:y,currentChapter:u,onChapterSelect:oe,currentScript:S,onScriptSelect:le,selectedTopicId:k,onTopicSelect:q,loading:i}),a.jsx("div",{className:"top-bar-right",children:a.jsx("button",{className:"theme-toggle",onClick:()=>se(!E),title:E?"切換到亮色模式":"切換到暗色模式",children:E?a.jsx(ue,{size:20}):a.jsx(fe,{size:20})})})]}),ce&&a.jsx("div",{className:"hydration-overlay",children:a.jsxs("div",{className:"hydration-card",children:[a.jsxs("div",{className:"hydration-header",children:[a.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),a.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),a.jsx("div",{className:"hydration-progress-container",children:a.jsx("div",{className:"hydration-progress-bar",style:{width:`${o}%`}})}),a.jsxs("div",{className:"hydration-status",children:[a.jsx("span",{children:h}),a.jsxs("span",{children:[o,"%"]})]})]})}),a.jsxs("div",{className:"panes-container",children:[a.jsx("div",{className:"content-pane",children:a.jsx(xe,{chapter:u,onCodeClick:ie,selectedTopicId:k,darkMode:E,output:F,isRunning:C,plotImages:O,onClearOutput:()=>{x(""),M([])}})}),S&&a.jsx("div",{className:"preview-pane",style:{width:`${W}px`},children:a.jsx(p.Suspense,{fallback:a.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:a.jsx(Be,{script:S,onClose:de,onRun:re,isRunning:C,isLoading:t,output:F,images:O,isInteractive:U,darkMode:E,width:W,onResize:ne})})})]})]}),a.jsx(ke,{pyodide:s})]})}he.createRoot(document.getElementById("root")).render(a.jsx(ye.StrictMode,{children:a.jsx(Qe,{})}));
