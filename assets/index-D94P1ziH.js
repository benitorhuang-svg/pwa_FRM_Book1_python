...
try:
    ...
    pass
`;let O=null,D=null;const Le=["https://cdn.jsdelivr.net/pyodide/v0.26.4/full/pyodide.js","https://unpkg.com/pyodide@0.26.4/pyodide.js"];function Ne(s){return new Promise((e,t)=>{const n=document.createElement("script");n.src=s,n.crossOrigin="anonymous",n.onload=e,n.onerror=()=>t(new Error(`Failed to load script: ${s}`)),document.head.appendChild(n)})}async function qe(){if(typeof window.loadPyodide!="function"){console.warn("window.loadPyodide not found, loading dynamically...");for(const s of Le)try{if(await Ne(s),typeof window.loadPyodide=="function"){console.log(`Pyodide loaded from ${s}`);return}}catch(e){console.warn(`Failed to load Pyodide from ${s}:`,e.message)}throw new Error("Failed to load Pyodide from all CDN sources")}}class $e{constructor(e){this.onProgress=e,this.current=0,this.target=0,this.message="",this.interval=null,this.step=.1}start(){this.interval||(this.interval=setInterval(()=>{if(this.current<this.target){const e=this.target-this.current,t=e>20?3.5:e>5?1.5:.5;this.current=Math.min(this.current+t,this.target),this.onProgress(Math.floor(this.current),this.message)}else this.target<99&&(this.current=Math.min(this.current+.08,99.9),this.onProgress(Math.floor(this.current),this.message))},100))}update(e,t){this.target=e,t&&(this.message=t)}async yieldToUI(){return new Promise(e=>setTimeout(e,30))}finish(e="Ready!"){this.interval&&clearInterval(this.interval),this.current=100,this.onProgress(100,e)}}async function Me(s){return O?(s&&s(100,"Ready"),O):D?D.then(e=>(s&&s(100,"Ready"),e)):(D=(async()=>{const e=new $e(s);e.start();try{e.update(10,"正在啟動 Python 直譯器 (v0.26.4)..."),await qe();let t=null,n=3;for(;n>0;)try{const l="https://cdn.jsdelivr.net/pyodide/v0.26.4/full/";console.log("Using Pyodide CDN for packages:",l),t=await window.loadPyodide({indexURL:l});break}catch(l){if(console.warn(`Failed to load Pyodide (attempts left: ${n-1}):`,l),n--,n===0)throw l;await new Promise(u=>setTimeout(u,1e3))}e.update(25,"引擎啟動完成，正在檢查本地暫存環境..."),await e.yieldToUI();const o="/lib/python3.12/site-packages";try{t.FS.mkdirTree(o)}catch{}t.FS.mount(t.FS.filesystems.IDBFS,{},o),e.update(26,"正在從本地儲存區載入 Python 環境..."),await new Promise(l=>{t.FS.syncfs(!0,u=>{u&&console.error("IDBFS sync error:",u),l()})}),await e.yieldToUI();const d=["numpy","pandas","matplotlib","scipy","statsmodels","sympy","lxml","micropip"],h=["numpy-financial","pandas-datareader","pyodide-http","pymoo==0.4.1","seaborn"],r=t.FS.readdir(o),y=d.filter(l=>!r.includes(l)&&!r.some(u=>u.startsWith(l+"-"))),c=h.filter(l=>{const u=l.replace("-","_");return!r.includes(l)&&!r.includes(u)&&!r.some(w=>w.startsWith(l+"-"))&&!r.some(w=>w.startsWith(u+"-"))});if(y.length>0||c.length>0){const l=[...y,...c];if(e.update(30,`正在同步並安裝套件: ${l.join(", ")}...`),y.length>0&&(e.update(40,`正在從 CDN 獲取核心科學套件 (${y.length} 個)...`),await t.loadPackage(y)),c.length>0){e.update(70,`正在掛載第三方擴充資源 (${c.length} 個)...`),await t.loadPackage("micropip");const u=t.pyimport("micropip"),w=c.map(k=>k==="numpy-financial"?"/pwa_FRM_Book1_python/wheels/numpy_financial-1.0.0-py3-none-any.whl":k);await u.install(w)}e.update(85,"正在固化環境映射至本地磁碟..."),await new Promise(u=>{t.FS.syncfs(!1,w=>{w&&console.error("IDBFS save error:",w),u()})}),e.update(90,"✅ 環境已完成本地持久化儲存")}else e.update(90,"🚀 偵測到本地環境快照，已跳過網路請求");return await e.yieldToUI(),e.update(92,"⚙️ 配置：正在載入環境優化補丁..."),await e.yieldToUI(),await Promise.all([t.runPythonAsync(`
import warnings
warnings.simplefilter("ignore", DeprecationWarning)
warnings.simplefilter("ignore", FutureWarning)
warnings.simplefilter("ignore", SyntaxWarning)
...
builtins.input = custom_input
            `),await e.yieldToUI(),O=t,D=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw D=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),D)}async function De(s,e,t=3e4){let n;const o=new Promise((d,h)=>{n=setTimeout(()=>{h(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),o])}finally{clearTimeout(n)}}async function Fe(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
                `)}catch{}}async function Ie(s){try{const e=await s.runPythonAsync(`
import matplotlib.pyplot as plt
len(plt.get_fignums())
...
if plt.style.available and 'default' in plt.style.available:
    ...
    `)}catch(t){console.error("✗ Matplotlib 初始化失敗:",t)}}async function Ce(s){try{await s.runPythonAsync(`
import matplotlib.pyplot as plt
...
`),$([]);try{await Fe(s);const m=f.includes("matplotlib.widgets")||f.includes("Slider")||f.includes("Button");ae(m);const _=m?"module://matplotlib_pyodide.wasm_backend":"AGG";if(H!==_&&(await J(s,m),G(_)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),m){const j=document.getElementById("pyodide-plot-container");j&&(j.innerHTML="",window.document.pyodideMplTarget=j)}await Y(f);try{await De(s,f)}catch(j){const L=await s.runPythonAsync("sys.stdout.getvalue()"),M=K(j);x((L?L+`
`:"")+M),R(!1),F.end("run-code");return}const S=await s.runPythonAsync("sys.stdout.getvalue()");if(x(S||"執行完成（無文字內容輸出 ）"),U)await Ce(s);else{const j=await Ie(s);$(j)}}catch(m){const _=K(m);x(_),console.error(m)}finally{R(!1),F.end("run-code")}}},oe=f=>{if(w(f),b(null),q(""),x(""),$([]),f&&f.examples&&s){const m=f.examples.map(_=>_.code).join(`
`);Y(m,!0)}},ie=f=>{b(f),x(""),$([])},le=f=>{b(f),x(""),$([])},de=()=>{b(null),x(""),$([])},ce=t;return a.jsxs("div",{className:`app ${E?"dark":""}`,children:[a.jsxs("div",{className:"main-content",children:[a.jsxs("div",{className:"top-bar",children:[a.jsxs("div",{className:"top-bar-left",children:[a.jsx(pe,{size:20,className:"logo-icon"}),a.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),a.jsx(we,{chapters:y,currentChapter:u,onChapterSelect:oe,currentScript:k,onScriptSelect:le,selectedTopicId:P,onTopicSelect:q,loading:i}),a.jsx("div",{className:"top-bar-right",children:a.jsx("button",{className:"theme-toggle",onClick:()=>se(!E),title:E?"切換到亮色模式":"切換到暗色模式",children:E?a.jsx(ue,{size:20}):a.jsx(fe,{size:20})})})]}),ce&&a.jsx("div",{className:"hydration-overlay",children:a.jsxs("div",{className:"hydration-card",children:[a.jsxs("div",{className:"hydration-header",children:[a.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),a.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),a.jsx("div",{className:"hydration-progress-container",children:a.jsx("div",{className:"hydration-progress-bar",style:{width:`${o}%`}})}),a.jsxs("div",{className:"hydration-status",children:[a.jsx("span",{children:h}),a.jsxs("span",{children:[o,"%"]})]})]})}),a.jsxs("div",{className:"panes-container",children:[a.jsx("div",{className:"content-pane",children:a.jsx(xe,{chapter:u,onCodeClick:ie,selectedTopicId:P,darkMode:E,output:C,isRunning:T,plotImages:W,onClearOutput:()=>{x(""),$([])}})}),k&&a.jsx("div",{className:"preview-pane",style:{width:`${z}px`},children:a.jsx(p.Suspense,{fallback:a.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:a.jsx(Ae,{script:k,onClose:de,onRun:re,isRunning:T,isLoading:t,output:C,images:W,isInteractive:U,darkMode:E,width:z,onResize:ne})})})]})]}),a.jsx(Pe,{pyodide:s})]})}he.createRoot(document.getElementById("root")).render(a.jsx(ye.StrictMode,{children:a.jsx(Qe,{})}));
