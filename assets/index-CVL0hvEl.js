...
import{r as u,j as a,C as B,_ as me,B as pe,S as ue,M as fe,R as ge,a as ye}from"./vendor-react-Dhy20vu4.js";import{B as he,p as _e}from"./vendor-utils-DahDrsZr.js";import{m as ve}from"./vendor-katex-DnHNbJWt.js";import"./vendor-Duqdn4fa.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))n(o);new MutationObserver(o=>{for(const d of o)if(d.type==="childList")for(const g of d.addedNodes)g.tagName==="LINK"&&g.rel==="modulepreload"&&n(g)}).observe(document,{childList:!0,subtree:!0});function t(o){const d={};return o.integrity&&(d.integrity=o.integrity),o.referrerPolicy&&(d.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?d.credentials="include":o.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function n(o){if(o.ep)return;o.ep=!0;const d=t(o);fetch(o.href,d)}})();function be({chapters:s,currentChapter:e,onChapterSelect:t,currentScript:n,onScriptSelect:o,selectedTopicId:d,onTopicSelect:g,loading:r}){var c;const y=u.useMemo(()=>{var w;const l=(w=e==null?void 0:e.content)==null?void 0:w.intro;if(!l)return[];let i="";if(typeof l=="string")i=l;else if(typeof l=="object"&&l.body)i=l.body;else return[];const p=/^###\s+(.+)$/gm,b=[];let S;for(;(S=p.exec(i))!==null;){const k=S[1].trim(),A="topic-"+k.replace(/\s+/g,"-").toLowerCase();b.push({id:A,title:k})}return b},[e]);return a.jsxs("div",{className:"top-nav-container",children:[a.jsx("div",{className:"nav-group",children:a.jsxs("div",{className:"custom-select-wrapper",children:[a.jsxs("select",{id:"chapter-select",name:"chapter-select",value:(e==null?void 0:e.id)||"",onChange:l=>{const i=s.find(p=>p.id===l.target.value);i&&t(i)},disabled:r||s.length===0,className:"custom-select",children:[a.jsx("option",{value:"",disabled:!0,children:r?"載入中...":"📖 章節選擇"}),s.map(l=>a.jsx("option",{value:l.id,children:l.title},l.id))]}),a.jsx(B,{className:"select-icon",size:16})]})}),a.jsx("div",{className:`nav-group ${!e||y.length===0?"disabled":""}`,children:a.jsxs("div",{className:"custom-select-wrapper",children:[a.jsxs("select",{id:"topic-select",name:"topic-select",value:d,onChange:l=>g(l.target.value),disabled:!e||y.length===0,className:"custom-select",children:[a.jsx("option",{value:"",children:"💡 重點導覽"}),y.map(l=>a.jsx("option",{value:l.id,children:l.title},l.id))]}),a.jsx(B,{className:"select-icon",size:16})]})}),a.jsx("div",{className:`nav-group ${e?"":"disabled"}`,children:a.jsxs("div",{className:"custom-select-wrapper",children:[a.jsxs("select",{id:"script-select",name:"script-select",value:(n==null?void 0:n.filename)||"",onChange:l=>{if(e!=null&&e.examples){const i=e.examples.find(p=>p.filename===l.target.value);i&&o(i)}},disabled:!e||!e.examples,className:"custom-select",children:[a.jsx("option",{value:"",disabled:!0,children:"💻 程式代碼"}),(c=e==null?void 0:e.examples)==null?void 0:c.map(l=>a.jsx("option",{value:l.filename,children:l.filename},l.filename))]}),a.jsx(B,{className:"select-icon",size:16})]})})]})}const we=new he(ve({throwOnError:!1,output:"html",nonStandard:!0})),xe=u.memo(({chapter:s,onCodeClick:e,selectedTopicId:t,output:n,isRunning:o,plotImages:d})=>{const g=u.useMemo(()=>{var y;if(!s)return null;const r=(y=s.content)==null?void 0:y.intro;if(r){let c="";typeof r=="string"?c=r:typeof r=="object"&&(c=`# ${r.title||""}
...
$$
`).replace(new RegExp("(?<!\\$)\\$(?!\\$)\\s*(.*?)\\s*(?<!\\$)\\$(?!\\$)","g"),"$$$1$");let l=we.parse(c);l=l.replace(/<h3>(.*?)<\/h3>/g,(w,k)=>`<h3 id="${"topic-"+k.replace(/<[^>]*>/g,"").trim().replace(/\s+/g,"-").toLowerCase()}">${k}</h3>`);let p=_e.sanitize(l,{ADD_TAGS:["math","annotation","semantics","mrow","msub","msup","msubsup","mover","munder","munderover","mmultiscripts","mprec","mnext","mtable","mtr","mtd","mfrac","msqrt","mroot","mstyle","merror","mpadded","mphantom","mfenced","menclose","ms","mglyph","maligngroup","malignmark","maction","svg","path","use","span","div"],ADD_ATTR:["id","target","xlink:href","class","style","aria-hidden","viewBox","d","fill","stroke","stroke-width","data-filename"]});return[...s.examples||[]].sort((w,k)=>k.filename.length-w.filename.length).forEach(w=>{const k=w.filename.replace(".","\\."),A=new RegExp(`(?<!['".\\w])(${k})(?!['".\\w])`,"g");p=p.replace(A,`<span class="code-link" data-filename="${w.filename}">${w.filename}</span>`)}),p}else{const c=s.examples||[];let l=`
        <div class="chapter-intro">
          <h2>${s.title}</h2>
          <p>本章包含 ${c.length} 個程式範例</p>
          <div class="example-grid">
      `;return c.forEach((i,p)=>{l+=`
          <div class="example-card">
            <div class="example-number">${p+1}</div>
            <div class="example-info">
              <h3>${i.title}</h3>
              <span class="code-link" data-filename="${i.filename}">${i.filename}</span>
            </div>
          </div>
        `}),l+=`
          </div>
        </div>
      `,l}},[s]);return u.useEffect(()=>{const r=y=>{if(y.target.classList.contains("code-link")){const c=y.target.dataset.filename;let l=null;s!=null&&s.examples&&(l=s.examples.find(i=>i.filename===c),l&&!l.metadata&&(l.metadata={description:l.title})),l&&e(l)}};return document.addEventListener("click",r),()=>document.removeEventListener("click",r)},[s,e]),u.useEffect(()=>{const r=document.querySelector(".content-scroll");r&&(r.scrollTop=0)},[s]),u.useEffect(()=>{if(t){const r=document.getElementById(t);r&&r.scrollIntoView({behavior:"smooth",block:"start"})}},[t]),u.useEffect(()=>{if(n||d&&d.length>0||o){const r=document.querySelector(".content-scroll");r&&r.scrollTo({top:0,behavior:"instant"})}},[n,d,o]),a.jsx("div",{className:"content-panel",children:a.jsx("div",{className:"content-scroll",children:n||d&&d.length>0||o?a.jsxs("div",{id:"execution-output",className:"execution-output-section",children:[o&&a.jsxs("div",{className:"running-indicator",children:[a.jsx("div",{className:"spinner"}),a.jsx("span",{children:"程式執行中..."})]}),n&&a.jsx("pre",{className:"output-text",children:n}),d&&d.length>0&&a.jsx("div",{className:"output-images",children:d.map((r,y)=>a.jsx("div",{className:"output-image",children:a.jsx("img",{src:r,alt:`Plot ${y+1}`})},y))})]}):g?a.jsx("div",{className:"markdown-body",dangerouslySetInnerHTML:{__html:g}}):a.jsx("div",{className:"welcome-screen",children:a.jsxs("div",{className:"welcome-card premium-welcome",children:[a.jsxs("div",{className:"welcome-brand",children:[a.jsx("img",{src:"book-cover.jpg",alt:"FRM Python 理論與實戰",className:"welcome-book-img"}),a.jsx("a",{href:"https://deepwisdom.com.tw/product/%e6%89%8b%e8%a1%93%e5%88%80%e8%88%ac%e7%b2%be%e6%ba%96%e7%9a%84frm-%e7%94%a8python%e7%a7%91%e5%ad%b8%e7%ae%a1%e6%8e%a7%e8%b2%a1%e9%87%91%e9%a2%a8%e9%9a%aa%e5%9f%ba%e7%a4%8e%e7%af%87dm2301/",target:"_blank",rel:"noopener noreferrer",className:"welcome-purchase-btn",children:"[ 本書官網購買連結 ]"})]}),a.jsxs("div",{className:"welcome-content",children:[a.jsxs("h2",{className:"welcome-title",children:["手術刀般精準的 FRM 用 Python",a.jsx("br",{}),"科學管控財金風險：基礎篇"]}),a.jsx("div",{className:"welcome-slogan",children:"☆★☆★【有如手術刀般精準！利用Python幫你管控財金風險！】★☆★☆"}),a.jsxs("div",{className:"welcome-text-scroll",children:[a.jsx("p",{children:"本書使用了當紅的程式語言 Python，從程式設計的基本觀念談起。沒有 Python 基礎也沒關係，完整的 Python 介紹，讓您能順利銜接資料科學家最常用的套件整理，包括 Numpy，以及特別針對格式化表格類處理的 Pandas，也充分介紹了 Dataframe 的各種應用。"}),a.jsx("p",{children:"在有了充足的資料之後，接著需要有可以展示數據的工具。除了大家最愛用的 Matplotlib 之外，也介紹了高手才會用的 Seaborn。當熟悉了工具之後，就正式進入了金融理論，包括基礎的機率及統計、各種模型及機率分佈，以及抽樣、信賴區間等內容說明。"}),a.jsx("p",{children:"最後則進入到金融領域，除了介紹各種計算的演算法、模型、術語，也結合了前面所學的 Python 及工具，並講解金融商品最重要的「固定收益分析」。"}),a.jsx("p",{children:"本書從科學下手，讓您精準了解金融原理，確保金錢不再陷入水深火熱之中，將是您從科學到金融領域最重要的橋樑。"})]})]})]})})})})});function ke(){return null}const Pe=`
# QuantLib (ql) 強大模擬層
...
try:
    ...
    pass
`;let Q=null,M=null;const Ae=["https://cdn.jsdelivr.net/pyodide/v0.26.4/full/pyodide.js","https://unpkg.com/pyodide@0.26.4/pyodide.js"];function qe(s){return new Promise((e,t)=>{const n=document.createElement("script");n.src=s,n.crossOrigin="anonymous",n.onload=e,n.onerror=()=>t(new Error(`Failed to load script: ${s}`)),document.head.appendChild(n)})}async function De(){if(typeof window.loadPyodide!="function"){console.warn("window.loadPyodide not found, loading dynamically...");for(const s of Ae)try{if(await qe(s),typeof window.loadPyodide=="function"){console.log(`Pyodide loaded from ${s}`);return}}catch(e){console.warn(`Failed to load Pyodide from ${s}:`,e.message)}throw new Error("Failed to load Pyodide from all CDN sources")}}class Me{constructor(e){this.onProgress=e,this.current=0,this.target=0,this.message="",this.interval=null,this.step=.1}start(){this.interval||(this.interval=setInterval(()=>{if(this.current<this.target){const e=this.target-this.current,t=e>20?3.5:e>5?1.5:.5;this.current=Math.min(this.current+t,this.target),this.onProgress(Math.floor(this.current),this.message)}else this.target<99&&(this.current=Math.min(this.current+.08,99.9),this.onProgress(Math.floor(this.current),this.message))},100))}update(e,t){this.target=e,t&&(this.message=t)}async yieldToUI(){return new Promise(e=>setTimeout(e,30))}finish(e="Ready!"){this.interval&&clearInterval(this.interval),this.current=100,this.onProgress(100,e)}}async function Ce(s){return Q?(s&&s(100,"Ready"),Q):M?M.then(e=>(s&&s(100,"Ready"),e)):(M=(async()=>{const e=new Me(s);e.start();try{e.update(10,"正在啟動 Python 直譯器 (v0.26.4)..."),await De();let t=null,n=3;for(;n>0;)try{const i="https://cdn.jsdelivr.net/pyodide/v0.26.4/full/";console.log("Using Pyodide CDN for packages:",i),t=await window.loadPyodide({indexURL:i});break}catch(i){if(console.warn(`Failed to load Pyodide (attempts left: ${n-1}):`,i),n--,n===0)throw i;await new Promise(p=>setTimeout(p,1e3))}try{const i=document.getElementById("inline-scipy-stub");if(i&&i.textContent&&i.textContent.trim().length>0)try{await window.pyodide.runPythonAsync(i.textContent),console.log("Inline SciPy stub executed early.")}catch(p){console.warn("Failed to execute inline SciPy stub:",p)}}catch(i){console.warn("Error checking inline SciPy stub:",i)}e.update(25,"引擎啟動完成，正在檢查本地暫存環境..."),await e.yieldToUI();const o="/lib/python3.12/site-packages";try{t.FS.mkdirTree(o)}catch{}t.FS.mount(t.FS.filesystems.IDBFS,{},o),e.update(26,"正在從本地儲存區載入 Python 環境..."),await new Promise(i=>{t.FS.syncfs(!0,p=>{p&&console.error("IDBFS sync error:",p),i()})}),await e.yieldToUI();const d=["numpy","pandas","matplotlib","scipy","statsmodels","sympy","lxml","micropip"],g=["numpy-financial","pandas-datareader","pyodide-http","pymoo==0.4.1","seaborn"],r=t.FS.readdir(o),y=d.filter(i=>!r.includes(i)&&!r.some(p=>p.startsWith(i+"-"))),c=g.filter(i=>{const p=i.replace("-","_");return!r.includes(i)&&!r.includes(p)&&!r.some(b=>b.startsWith(i+"-"))&&!r.some(b=>b.startsWith(p+"-"))});if(y.length>0||c.length>0){const i=[...y,...c];if(e.update(30,`正在同步並安裝套件: ${i.join(", ")}...`),y.length>0&&(e.update(40,`正在從 CDN 獲取核心科學套件 (${y.length} 個)...`),await t.loadPackage(y)),c.length>0){e.update(70,`正在掛載第三方擴充資源 (${c.length} 個)...`),await t.loadPackage("micropip");const p=t.pyimport("micropip"),b=c.map(S=>S==="numpy-financial"?"/pwa_FRM_Book1_python/wheels/numpy_financial-1.0.0-py3-none-any.whl":S);await p.install(b)}e.update(85,"正在固化環境映射至本地磁碟..."),await new Promise(p=>{t.FS.syncfs(!1,b=>{b&&console.error("IDBFS save error:",b),p()})}),e.update(90,"✅ 環境已完成本地持久化儲存")}else e.update(90,"🚀 偵測到本地環境快照，已跳過網路請求");return await e.yieldToUI(),e.update(92,"⚙️ 配置：正在載入環境優化補丁..."),await e.yieldToUI(),await Promise.all([t.runPythonAsync(`
import warnings
warnings.simplefilter("ignore", DeprecationWarning)
warnings.simplefilter("ignore", FutureWarning)
warnings.simplefilter("ignore", SyntaxWarning)
...
try:
    ...
    if not hasattr(scipy.stats, 'binom_test') and hasattr(scipy.stats, 'binomtest'):
        ...
        scipy.stats.binom_test = binom_test_shim
                ...
                `)]),await e.yieldToUI(),e.update(98,"🚀 系統：正在啟動 Pymoo & QuantLib 虛擬層..."),await e.yieldToUI(),await Promise.all([t.runPythonAsync(je),t.runPythonAsync(Le),t.runPythonAsync(Ne),t.runPythonAsync(Pe),t.runPythonAsync(Se),t.runPythonAsync(Ee)]),e.update(99,"🛡️ 系統：權限校正與輸入掛鉤注入..."),await t.runPythonAsync(`
import builtins
import js
def custom_input(prompt = ""):
    try:
...
builtins.input = custom_input
            `),await e.yieldToUI(),Q=t,M=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw M=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),M)}async function Ie(s,e,t=3e4){let n;const o=new Promise((d,g)=>{n=setTimeout(()=>{g(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),o])}finally{clearTimeout(n)}}async function Te(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
                `)}catch{}}async function $e(s){try{const e=await s.runPythonAsync(`
import matplotlib.pyplot as plt
len(plt.get_fignums())
...
• 逐行執行程式碼找出問題
• 使用 print() 輸出變數值
• 簡化程式碼，逐步測試`}class ze{constructor(){this.metrics={},this.enabled=!0}start(e){this.enabled&&(this.metrics[e]={start:performance.now(),end:null,duration:null})}end(e){if(!this.enabled||!this.metrics[e])return 0;const t=performance.now(),n=this.metrics[e].start,o=t-n;return this.metrics[e].end=t,this.metrics[e].duration=o,this.log(e,o),o}log(e,t){const n=t<100?"⚡":t<1e3?"⏱️":"🐌",o=t<100?"color: green":t<1e3?"color: orange":"color: red";console.log(`%c${n} ${e}: ${t.toFixed(2)}ms`,o),t>1e3&&console.warn(`⚠️ ${e} 執行時間過長: ${t.toFixed(2)}ms`)}async measure(e,t){this.start(e);try{return await t()}finally{this.end(e)}}getMetric(e){return this.metrics[e]||null}getMetrics(){return{...this.metrics}}getStats(){const e=Object.values(this.metrics).filter(t=>t.duration!==null).map(t=>t.duration);return e.length===0?{count:0,total:0,average:0,min:0,max:0}:{count:e.length,total:e.reduce((t,n)=>t+n,0),average:e.reduce((t,n)=>t+n,0)/e.length,min:Math.min(...e),max:Math.max(...e)}}clear(e){e?delete this.metrics[e]:this.metrics={}}setEnabled(e){this.enabled=e}report(){const e=this.getStats();console.group("📊 效能報告"),console.log(`總計測量: ${e.count} 次`),console.log(`總時間: ${e.total.toFixed(2)}ms`),console.log(`平均時間: ${e.average.toFixed(2)}ms`),console.log(`最快: ${e.min.toFixed(2)}ms`),console.log(`最慢: ${e.max.toFixed(2)}ms`),console.groupEnd(),console.group("📋 詳細指標"),Object.entries(this.metrics).forEach(([t,n])=>{n.duration!==null&&console.log(`${t}: ${n.duration.toFixed(2)}ms`)}),console.groupEnd()}}const C=new ze;C.setEnabled(!1);let X=!1;function Re(){if(!X&&(X=!0,"PerformanceObserver"in window)){new PerformanceObserver(o=>{const d=o.getEntries(),g=d[d.length-1];console.log("🎨 LCP:",g.renderTime||g.loadTime)}).observe({entryTypes:["largest-contentful-paint"]}),new PerformanceObserver(o=>{o.getEntries().forEach(g=>{console.log("⚡ FID:",g.processingStart-g.startTime)})}).observe({entryTypes:["first-input"]});let t=0;new PerformanceObserver(o=>{o.getEntries().forEach(g=>{g.hadRecentInput||(t+=g.value)}),console.log("📐 CLS:",t)}).observe({entryTypes:["layout-shift"]})}}const Be=u.lazy(()=>me(()=>import("./CodePreviewPanel-BAmgNmSY.js"),__vite__mapDeps([0,1,2,3]))),ee={pandas:"pandas",matplotlib:"matplotlib",scipy:"scipy",statsmodels:"statsmodels",sympy:"sympy",autograd:"autograd",lxml:"lxml",openpyxl:"openpyxl",requests:"requests",numpy_financial:"wheels/numpy_financial-1.0.0-py3-none-any.whl",seaborn:"wheels/seaborn-0.13.2-py3-none-any.whl",pymoo:"wheels/pymoo-0.4.1-py3-none-any.whl",pandas_datareader:"wheels/pandas_datareader-0.10.0-py3-none-any.whl",pyodide_http:"wheels/pyodide_http-0.2.2-py3-none-any.whl",mpl_toolkits:"matplotlib",pylab:"matplotlib"},te={requests:["certifi","charset_normalizer","idna","urllib3"],certifi:"wheels/certifi-2026.1.4-py3-none-any.whl",charset_normalizer:"wheels/charset_normalizer-3.4.4-py3-none-any.whl",idna:"wheels/idna-3.11-py3-none-any.whl",urllib3:"wheels/urllib3-2.6.3-py3-none-any.whl"};function Qe(){const[s,e]=u.useState(null),[t,n]=u.useState(!0),[o,d]=u.useState(0),[g,r]=u.useState("初始化中..."),[y,c]=u.useState([]),[l,i]=u.useState(!0),[p,b]=u.useState(null),[S,w]=u.useState(null),[k,A]=u.useState(""),[T,x]=u.useState(""),[O,q]=u.useState([]),[$,F]=u.useState(!1),[U,ae]=u.useState(!1),[E,se]=u.useState(()=>{const f=localStorage.getItem("theme");return f==="dark"||!f&&!1}),[W,ne]=u.useState(600),[z]=u.useState(new Set),[G,H]=u.useState(null);u.useEffect(()=>{i(!0);const f=`/pwa_FRM_Book1_python/data/chapters.json?t=${Date.now()}`;fetch(f).then(m=>m.json()).then(m=>{m&&m.length>0&&c(m),i(!1)}).catch(m=>{console.error("Failed to load chapters:",m),i(!1)})},[]),u.useEffect(()=>{Ce((m,_)=>{d(m),r(_)}).then(m=>{e(m),n(!1),C.end("pyodide-init")}),Re();const f=[];if("serviceWorker"in navigator){const m=setInterval(()=>{navigator.serviceWorker.getRegistrations().then(P=>{P.forEach(N=>N.update())})},36e5),_=()=>{};navigator.serviceWorker.addEventListener("controllerchange",_),f.push(()=>{clearInterval(m),navigator.serviceWorker.removeEventListener("controllerchange",_)})}if("storage"in navigator&&"estimate"in navigator.storage){const m=async()=>{try{const P=await navigator.storage.estimate();P.usage&&P.quota&&P.usage/P.quota*100>80&&console.warn("Storage usage is high (>80%). Consider cleaning up.")}catch(P){console.warn("Failed to check storage quota:",P)}};m();const _=setInterval(m,300*1e3);f.push(()=>clearInterval(_))}return()=>{f.forEach(m=>m())}},[]),u.useEffect(()=>{localStorage.setItem("theme",E?"dark":"light"),document.documentElement.setAttribute("data-theme",E?"dark":"light")},[E]);const V=async(f,m=!1)=>{var Z;if(!s)return;const _=f.match(/^\s*(?:from|import)\s+([a-zA-Z0-9_]+)/gm);if(!_)return;const P=["sys","os","io","time","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib"],N=["numpy","pandas","matplotlib","scipy","statsmodels","sympy","lxml","micropip","js","builtins","QuantLib"],j=[...new Set(_.map(v=>{const h=v.trim().split(/\s+/);return h[0]==="from",h[1].split(".")[0]}))].filter(v=>!P.includes(v)&&!N.includes(v)).filter(v=>!z.has(v));if(j.length===0)return;const D=[],Y=new URL("/pwa_FRM_Book1_python/",window.location.origin).href;if(j.forEach(v=>{const h=ee[v];h?(D.push(h.endsWith(".whl")?Y+h:h),(te[v]||[]).forEach(I=>{const R=te[I]||I;D.push(R.endsWith(".whl")?Y+R:R)})):D.push(v)}),D.length>0)try{const v=[...new Set(D)];m||x(h=>h+`正在動態載入所需套件 [${j.join(", ")}]...
`),await s.loadPackage("micropip"),await s.runPythonAsync(`
import micropip
await micropip.install(${JSON.stringify(v)}, keep_going=True)
        `),j.includes("matplotlib")&&(await J(s),G||H("AGG")),j.forEach(h=>{z.add(h);const L=ee[h];L&&!L.endsWith(".whl")&&z.add(L)}),m||x(h=>h+`✅ 套件載入完成。
`)}catch(v){if(console.warn("Dependency loading failed:",v),!m){const h=v.message||String(v);if(h.includes("Can't find a pure Python 3 wheel")){const L=((Z=h.match(/for: '([^']+)'/))==null?void 0:Z[1])||"unknown";x(I=>I+`⚠️ 套件 "${L}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
`)}else x(L=>L+`⚠️ 套件載入出現問題，嘗試直接執行...
`)}}},re=async f=>{if(!(!s||$||!f)){C.start("run-code"),F(!0),x(`執行中...
`),q([]);try{await Te(s);const m=f.includes("matplotlib.widgets")||f.includes("Slider")||f.includes("Button");ae(m);const _=m?"module://matplotlib_pyodide.wasm_backend":"AGG";if(G!==_&&(await J(s,m),H(_)),await s.runPythonAsync(`
import sys
from io import StringIO
sys.stdout = StringIO()
      `),m){const N=document.getElementById("pyodide-plot-container");N&&(N.innerHTML="",window.document.pyodideMplTarget=N)}await V(f);try{await Ie(s,f)}catch(N){const j=await s.runPythonAsync("sys.stdout.getvalue()"),D=K(N);x((j?j+`
`:"")+D),F(!1),C.end("run-code");return}const P=await s.runPythonAsync("sys.stdout.getvalue()");if(x(P||"執行完成（無文字內容輸出 ）"),U)await Fe(s);else{const N=await $e(s);q(N)}}catch(m){const _=K(m);x(_),console.error(m)}finally{F(!1),C.end("run-code")}}},oe=f=>{if(b(f),w(null),A(""),x(""),q([]),f&&f.examples&&s){const m=f.examples.map(_=>_.code).join(`
`);V(m,!0)}},ie=f=>{w(f),x(""),q([])},le=f=>{w(f),x(""),q([])},de=()=>{w(null),x(""),q([])},ce=t;return a.jsxs("div",{className:`app ${E?"dark":""}`,children:[a.jsxs("div",{className:"main-content",children:[a.jsxs("div",{className:"top-bar",children:[a.jsxs("div",{className:"top-bar-left",children:[a.jsx(pe,{size:20,className:"logo-icon"}),a.jsx("span",{className:"app-title",children:"FRM_Book1 (基礎篇)"})]}),a.jsx(be,{chapters:y,currentChapter:p,onChapterSelect:oe,currentScript:S,onScriptSelect:le,selectedTopicId:k,onTopicSelect:A,loading:l}),a.jsx("div",{className:"top-bar-right",children:a.jsx("button",{className:"theme-toggle",onClick:()=>se(!E),title:E?"切換到亮色模式":"切換到暗色模式",children:E?a.jsx(ue,{size:20}):a.jsx(fe,{size:20})})})]}),ce&&a.jsx("div",{className:"hydration-overlay",children:a.jsxs("div",{className:"hydration-card",children:[a.jsxs("div",{className:"hydration-header",children:[a.jsx("div",{className:"hydration-title",children:"FRM Python 引擎啟動中"}),a.jsx("div",{className:"hydration-subtitle",children:"Financial Risk Management"})]}),a.jsx("div",{className:"hydration-progress-container",children:a.jsx("div",{className:"hydration-progress-bar",style:{width:`${o}%`}})}),a.jsxs("div",{className:"hydration-status",children:[a.jsx("span",{children:g}),a.jsxs("span",{children:[o,"%"]})]})]})}),a.jsxs("div",{className:"panes-container",children:[a.jsx("div",{className:"content-pane",children:a.jsx(xe,{chapter:p,onCodeClick:ie,selectedTopicId:k,darkMode:E,output:T,isRunning:$,plotImages:O,onClearOutput:()=>{x(""),q([])}})}),S&&a.jsx("div",{className:"preview-pane",style:{width:`${W}px`},children:a.jsx(u.Suspense,{fallback:a.jsx("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",height:"100%",color:"#888"},children:"Loading Editor..."}),children:a.jsx(Be,{script:S,onClose:de,onRun:re,isRunning:$,isLoading:t,output:T,images:O,isInteractive:U,darkMode:E,width:W,onResize:ne})})})]})]}),a.jsx(ke,{pyodide:s})]})}async function Oe(){try{if(!("serviceWorker"in navigator))return;const s="frm_sw_cleanup_done_v1";if(localStorage.getItem(s))return;if(!navigator.serviceWorker.controller){localStorage.setItem(s,"1");return}console.info("[AUTO-CLEAN] Detected active Service Worker — cleaning caches and storage.");try{const e=await navigator.serviceWorker.getRegistrations();await Promise.all(e.map(t=>t.unregister().catch(()=>{}))),console.info("[AUTO-CLEAN] serviceWorker registrations unregistered.")}catch(e){console.warn("[AUTO-CLEAN] Failed to unregister service workers",e)}try{if("caches"in window){const e=await caches.keys();await Promise.all(e.map(t=>caches.delete(t).catch(()=>{}))),console.info("[AUTO-CLEAN] CacheStorage cleared:",e)}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear CacheStorage",e)}try{if("databases"in indexedDB){const e=await indexedDB.databases();await Promise.all(e.map(t=>indexedDB.deleteDatabase(t.name).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB databases deleted:",e.map(t=>t.name))}else{const e=["pyodide","emscripten-archives","idb-filesystem","file_storage"];await Promise.all(e.map(t=>indexedDB.deleteDatabase(t).catch(()=>{}))),console.info("[AUTO-CLEAN] IndexedDB fallback delete attempted for candidates.")}}catch(e){console.warn("[AUTO-CLEAN] Failed to clear IndexedDB",e)}try{localStorage.clear(),sessionStorage.clear(),console.info("[AUTO-CLEAN] localStorage and sessionStorage cleared.")}catch(e){console.warn("[AUTO-CLEAN] Failed to clear storage",e)}try{localStorage.setItem(s,"1")}catch{}try{console.info("[AUTO-CLEAN] Reloading page to load fresh assets."),window.location.reload(!0)}catch(e){console.info("[AUTO-CLEAN] Reload skipped.",e)}}catch(s){console.warn("[AUTO-CLEAN] Unexpected error",s)}}Oe().catch(()=>{});ge.createRoot(document.getElementById("root")).render(a.jsx(ye.StrictMode,{children:a.jsx(Qe,{})}));
