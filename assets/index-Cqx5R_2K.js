...
await micropip.install(${JSON.stringify(w)}, keep_going=True)
          `)),j.includes("pandas_datareader")&&await s.runPythonAsync(me),j.includes("matplotlib")&&(await ie(s),X||ee("AGG")),j.forEach(S=>{M.add(S);const N=K[S];N&&!N.endsWith(".whl")&&M.add(N)}),p||g(S=>S+`✅ 套件載入完成。
`)}catch(h){if(console.warn("Dependency loading failed:",h),window.failedPackages||(window.failedPackages=new Set),j.forEach(w=>window.failedPackages.add(w)),!p){const w=h.message||String(h);if(w.includes("Can't find a pure Python 3 wheel")){const S=((ne=w.match(/for: '([^']+)'/))==null?void 0:ne[1])||"unknown";g(N=>N+`⚠️ 套件 "${S}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
